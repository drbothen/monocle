...
### AC-001 (traces to BC-2.01.008 postcondition 1 — auth token already generated by S-006/monocle-auth)
The daemon's lock file `authToken` field contains exactly a 64-character lowercase hexadecimal
string generated by `monocle-auth::generate_session_token()` using `rand::rngs::OsRng`
(NOT `thread_rng`; EXACT pin `=0.8.6`). Token regex: `/^[0-9a-f]{64}$/`. NFR-004 validation.
S-009 reads this token from the already-written lock file (Decision 2: token generation is S-006's deliverable).
...
- [ ] In `monocle-runtime/src/auth.rs`, read the 64-hex token from the lock file written by S-006
  ...
  - `monocle-auth::generate_session_token()` is S-006's deliverable; S-009 is the consumer
...
S-006 (Wave 2): Lock file writer complete; `monocle-auth::generate_session_token()` already
called at `DaemonLock::acquire()` time (Decision 2). The 64-hex token is already written to
