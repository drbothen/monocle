...
- All 22 Phase 1 BCs — 6 daemon-endpoint BCs formalized in PRD v1.16
  (BC-DAEMON-001..006) plus 16 BCs pre-staged across `SS-daemon-lifecycle.md`
  v1.0.17 (BC-RING-001, BC-AUTH-001, BC-AUTH-002, BC-LOCK-001),
...
|-------|-----------|-----------------|-------------------|---------------------|
| VP-DAEMON-001 | BC-DAEMON-001 (PRD v1.16 / SS-daemon-lifecycle v1.0.17) | `/healthz` unauthenticated 200/503 with uptime + version | unit-test | — |
| VP-DAEMON-002 | BC-DAEMON-002 (PRD v1.16 / SS-daemon-lifecycle v1.0.17) | `/status` authenticated daemon-state JSON with 10 required fields incl `abi_version: 1` | unit-test | — |
| VP-DAEMON-003 | BC-DAEMON-003 (PRD v1.16 / SS-daemon-lifecycle v1.0.17) | 256 KiB request-body limit; HTTP 413 + `payload_too_large` body on excess | unit-test | fuzz |
| VP-DAEMON-004 | BC-DAEMON-004 (PRD v1.16 / SS-daemon-lifecycle v1.0.17) | 10-second graceful drain on SIGTERM / SIGINT / `POST /shutdown`; 503 + `Retry-After: 10` on new hook POSTs during drain; 5-code POSIX exit taxonomy (0/130/143/2/1) per cause | unit-test | — |
| VP-DAEMON-005 | BC-DAEMON-005 (PRD v1.16 / SS-daemon-lifecycle v1.0.17) | 4-path runtime-dir resolution chain (MONOCLE_RUNTIME_DIR env → ProjectDirs::runtime_dir() Linux → ProjectDirs::data_local_dir() macOS/Windows → DaemonStartError::RuntimeDirUnresolvable fail-fast); lock-file lifecycle atomically via `tempfile::persist`; pid-liveness gate; mode 0o600 lock-file + 0o700 runtime-dir owner-only (defense-in-depth per BC-DAEMON-005 Postcondition 8 / VP-DAEMON-005 Post-condition 9); removed on clean shutdown | unit-test | mutation-test |
| VP-DAEMON-006 | BC-DAEMON-006 (PRD v1.16 / SS-daemon-lifecycle v1.0.17) | Crash-recovery checkpoint JSON written before lock removal with mandatory millisecond `shutdown_utc` (YYYY-MM-DDTHH:MM:SS.sssZ); 60-second TUI offer window; cleanup on accept/decline/timeout | unit-test | — |
| VP-RING-001 | BC-RING-001 (SS-daemon-lifecycle v1.0.17) | JSONL ring record format-version is first key | unit-test | mutation-test |
| VP-AUTH-001 | BC-AUTH-001 (SS-daemon-lifecycle v1.0.17) | Wire format `monocle-v1:<64-hex>`; constant-time comparison | unit-test | fuzz |
| VP-AUTH-002 | BC-AUTH-002 (SS-daemon-lifecycle v1.0.17) | Two-body taxonomy: absent header → `missing_auth_token`; any value-present failure → `invalid_auth_token` (collapsed) | unit-test | fuzz |
| VP-LOCK-001 | BC-LOCK-001 (SS-daemon-lifecycle v1.0.17) | Lock-file `contract_version: 1` first key; readers gate on field | unit-test | mutation-test |
...
**Traces to:** BC-DAEMON-001 (PRD v1.16 §BC-DAEMON-001; SS-daemon-lifecycle.md
v1.0.17 §Health and Status Endpoints).
...
**Traces to:** BC-DAEMON-002 (PRD v1.16 §BC-DAEMON-002; SS-daemon-lifecycle.md
v1.0.17 §Health and Status Endpoints).
...
(per PRD v1.16 §BC-DAEMON-002, Verification subsection).

---

### §VP-DAEMON-003 — 256 KiB Body Limit; HTTP 413 on Excess

**Traces to:** BC-DAEMON-003 (PRD v1.16 §BC-DAEMON-003; SS-daemon-lifecycle.md
v1.0.17 §Body Size Limit).
...
**Traces to:** BC-DAEMON-004 (PRD v1.16 §BC-DAEMON-004; SS-daemon-lifecycle.md
v1.0.17 §Daemon Lifecycle Protocol §Shutdown Signal Handling, §Drain, and §Hard Shutdown).
...
6. **Exit-code taxonomy (5-code POSIX 128+N per PRD v1.16 §BC-DAEMON-004
...
- `tokio::signal::unix::signal(SignalKind::terminate())` is the SIGTERM
  ...
  selection (per arch v1.0.17 §Hard Shutdown step 6d).
...
6. **5-code POSIX exit taxonomy probe matrix (per PRD v1.16 §BC-DAEMON-004
...
(exit-code 5-code POSIX taxonomy probes per PRD v1.16 §BC-DAEMON-004
...
- `test_BC_DAEMON_004_exit_codes_posix_distinct` (per PRD v1.16
...
**Traces to:** BC-DAEMON-005 (PRD v1.16 §BC-DAEMON-005; SS-daemon-lifecycle.md
v1.0.17 §Daemon Lifecycle Protocol §Start Sequence and §Hard Shutdown;
F-R70-1 closure — hybrid runtime-dir resolution chain disposition (c)).

**Mechanical property:**

0. **Runtime-dir resolution chain (per PRD v1.16 §BC-DAEMON-005 precondition
   2 + arch v1.0.17 §Start Sequence step 1; F-R70-1 closure):** before any
...
7. **Asymmetry with BC-ENGINE-002-ERR `HomeUnresolvable` is intentional**
   (per PRD v1.16 §BC-DAEMON-005 invariant 4 + arch v1.0.17 §Start
...
4. Daemon graceful shutdown via synthetic SIGTERM → after drain
   ...
   post-drain).
5. **4-path resolution chain probe matrix (per PRD v1.16 §BC-DAEMON-005
...
9. **Runtime-dir mode 0o700 owner-only enforcement (per PRD v1.16
   §BC-DAEMON-005 Postcondition 8 + EC-052 + arch v1.0.17 §Start
   Sequence step 1; F-R75-1 VP-side closure + F-R79-3 PRD-side
   lift_invariants_to_bcs closure that elevated the 0o700 contract
   from EC-052 to §Postcondition tier in PRD v1.16 commit cd6541f —
...
probes via the canonical test-vector matrix in PRD v1.16 §BC-DAEMON-005
...
**Traces to:** BC-DAEMON-006 (PRD v1.16 §BC-DAEMON-006; SS-daemon-lifecycle.md
v1.0.17 §Daemon Lifecycle Protocol §Crash Recovery).
...
(per PRD v1.16 §BC-DAEMON-006, Verification subsection).
...
**Test name:** `test_BC_RING_001_format_version_first_key` (per PRD v1.16
...
(per PRD v1.16 §BC-AUTH-001, Verification subsection).

---

### §VP-AUTH-002 — Auth Header Two-Body Taxonomy: `missing_auth_token` vs `invalid_auth_token`

**Traces to:** BC-AUTH-002 (PRD v1.16 §BC-AUTH-002; SS-daemon-lifecycle.md
v1.0.17 §Daemon Lifecycle Protocol §Start Sequence; architect adjudication
...
(per PRD v1.16 §BC-AUTH-002, Verification subsection).
...
**Test name:** `test_BC_LOCK_001_contract_version_first_key` (per PRD v1.16
...
PRD v1.16 §BC-ABI-001, Verification subsection).
...
**Test name:** `test_BC_ABI_002_abi_version_const_exported` (per PRD v1.16
...
`monocle-core/tests/enum_audit.rs` per PRD v1.16 §BC-TYPES-001 invariant 1);
...
(per PRD v1.16 §BC-FACTORY-001, Verification subsection).
...
(per PRD v1.16 §BC-FACTORY-002, Verification subsection).
...
**Test name:** `test_BC_PROTO_001b_schema_version_rust_field` (per PRD v1.16
...
**Traces to:** BC-PROTO-002 (PRD v1.16 §BC-PROTO-002; SS-core-types-and-abi.md
...
architecture artifact specifies a Phase 1 dispatcher; PRD v1.16
...
PRD v1.16 §BC-PROTO-002 (Phase 4 test name `test_BC_PROTO_002_schema_version_validation_skip_unknown`
documented in PRD v1.16 §BC-PROTO-002 Verification subsection for Phase 4
...
(per PRD v1.16 §BC-ENGINE-001, Verification subsection).
...
(per PRD v1.16 §BC-ENGINE-002, Verification subsection).
...
SS-deps-pin-manifest.md v1.1.12 pin).
...
(per PRD v1.16 §BC-ENGINE-002-ERR, Verification subsection).
...
(per PRD v1.16 §BC-ENGINE-003, Verification subsection — hybrid name
...
|-------|----------------|-------|---------------------|-------------------|
| BC-DAEMON-001 | PRD v1.16 / SS-daemon-lifecycle.md v1.0.17 | VP-DAEMON-001 | unit-test | `monocle-runtime/tests/healthz_endpoint.rs` |
| BC-DAEMON-002 | PRD v1.16 / SS-daemon-lifecycle.md v1.0.17 | VP-DAEMON-002 | unit-test | `monocle-runtime/tests/status_endpoint_auth.rs` |
| BC-DAEMON-003 | PRD v1.16 / SS-daemon-lifecycle.md v1.0.17 | VP-DAEMON-003 | unit-test | `monocle-runtime/tests/body_size_limit.rs` |
| BC-DAEMON-004 | PRD v1.16 / SS-daemon-lifecycle.md v1.0.17 | VP-DAEMON-004 | unit-test | `monocle-runtime/tests/graceful_shutdown.rs` + `monocle-runtime/tests/daemon_lifecycle.rs` |
| BC-DAEMON-005 | PRD v1.16 / SS-daemon-lifecycle.md v1.0.17 | VP-DAEMON-005 | unit-test | `monocle-runtime/tests/lock_file_lifecycle.rs` |
| BC-DAEMON-006 | PRD v1.16 / SS-daemon-lifecycle.md v1.0.17 | VP-DAEMON-006 | unit-test | `monocle-runtime/tests/crash_recovery.rs` |
| BC-RING-001 | SS-daemon-lifecycle.md v1.0.17 | VP-RING-001 | unit-test | `monocle-runtime/tests/jsonl_ring.rs` |
| BC-AUTH-001 | SS-daemon-lifecycle.md v1.0.17 | VP-AUTH-001 | unit-test | `monocle-runtime/tests/auth_token_lifecycle.rs` |
| BC-AUTH-002 | SS-daemon-lifecycle.md v1.0.17 | VP-AUTH-002 | unit-test | `monocle-runtime/tests/auth_header_rejection.rs` |
| BC-LOCK-001 | SS-daemon-lifecycle.md v1.0.17 | VP-LOCK-001 | unit-test | `monocle-runtime/tests/lock_file_contract.rs` |
...
- **NFR-001 — hook latency p99 ≤ 300 ms (`PreToolUse`, `Stop`,
  `SessionStart`, `UserPromptSubmit` hook surfaces per PRD v1.16
  §NFR-001).** Per brief §Success Criteria latency target and PRD v1.16
...
- **NFR-002 — hook latency p99 ≤ 2000 ms (`Notification` hook surface,
  per PRD v1.16 §NFR-002 — scoped to `/hooks/notification` only;
...
- **NFR-003 — permission overlay first-paint ≤ 100 ms.** Per PRD v1.16
...
reference; it is NOT a Phase 1 monocle BC in PRD v1.16 §2.1 or §7 RTM
...
- **NFR-006 — bounded event bus with visible drop counter; 1000
  ...
  status bar.** Per PRD v1.16 §NFR-006 specification (PRD line 1208);
...
1. `.factory/specs/prd.md` v1.16 (commit cd6541f) — canonical BC source
   ...
   adjudications; PRD v1.16 is the F-R86 fix-burst closure (commit
   cd6541f) — I-R86-2 count fix at PRD §Trace v1.16 line 2470 `4 rows`
   → `5 rows`; SE-16a/b application documented in PRD §Trace v1.16;
   predecessor PRD v1.15 was the F-R85-IMP-2 + Extension 16 backfill
...
2. `.factory/specs/architecture/SS-daemon-lifecycle.md` v1.0.17 (commit
   a798d51) — source of BC-DAEMON-001..006, BC-RING-001, BC-AUTH-001,
   ...
   current canonical arch source per F-R83 fix-burst closure chain;
   §BC Summary footer extended to enumerate the 0o700 runtime-dir
   owner-only mode contract at the arch summary-table sibling site
   alongside the existing 0o600 lock-file mode enumeration per
   lift_invariants_to_bcs sibling-site propagation discipline /
   L-F-R63 Extension 14).
