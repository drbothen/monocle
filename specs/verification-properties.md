...
the 22 Behavioral Contracts (BCs) formalized in the Phase 1 PRD v1.24 (commit
a71ca67) and pre-staged across the Phase 1 architecture artifacts. Each VP
...
- All 22 Phase 1 BCs — 6 daemon-endpoint BCs formalized in PRD v1.24
  (BC-DAEMON-001..006) plus 16 BCs pre-staged across `SS-daemon-lifecycle.md`
  v1.0.24 (BC-RING-001, BC-AUTH-001, BC-AUTH-002, BC-LOCK-001),
...
|-------|-----------|-----------------|-------------------|---------------------|
| VP-DAEMON-001 | BC-DAEMON-001 (PRD v1.24 / SS-daemon-lifecycle v1.0.24) | `/healthz` unauthenticated 200/503 with uptime + version | integration-test | — |
| VP-DAEMON-002 | BC-DAEMON-002 (PRD v1.24 / SS-daemon-lifecycle v1.0.24) | `/status` authenticated daemon-state JSON with 10 required fields incl `abi_version: 1` | integration-test | — |
| VP-DAEMON-003 | BC-DAEMON-003 (PRD v1.24 / SS-daemon-lifecycle v1.0.24) | 256 KiB request-body limit; HTTP 413 + `payload_too_large` body on excess | integration-test | fuzz |
| VP-DAEMON-004 | BC-DAEMON-004 (PRD v1.24 / SS-daemon-lifecycle v1.0.24) | 10-second graceful drain on SIGTERM / SIGINT / `POST /shutdown`; 503 + `Retry-After: 10` on new hook POSTs during drain; 5-code POSIX exit taxonomy (0/130/143/2/1) per cause | integration-test | — |
| VP-DAEMON-005 | BC-DAEMON-005 (PRD v1.24 / SS-daemon-lifecycle v1.0.24) | 4-path runtime-dir resolution chain (MONOCLE_RUNTIME_DIR env → ProjectDirs::runtime_dir() Linux → ProjectDirs::data_local_dir() macOS/Windows → DaemonStartError::RuntimeDirUnresolvable fail-fast); lock-file lifecycle atomically via `tempfile::persist`; pid-liveness gate; mode 0o600 lock-file + 0o700 runtime-dir owner-only (defense-in-depth per BC-DAEMON-005 Postcondition 8 / VP-DAEMON-005 Post-condition 9); removed on clean shutdown | integration-test | mutation-test |
| VP-DAEMON-006 | BC-DAEMON-006 (PRD v1.24 / SS-daemon-lifecycle v1.0.24) | Crash-recovery checkpoint JSON written before lock removal with mandatory millisecond `shutdown_utc` (YYYY-MM-DDTHH:MM:SS.sssZ); 60-second TUI offer window; cleanup on accept/decline/timeout | integration-test | — |
| VP-RING-001 | BC-RING-001 (SS-daemon-lifecycle v1.0.24) | JSONL ring record format-version is first key | integration-test | mutation-test |
| VP-AUTH-001 | BC-AUTH-001 (SS-daemon-lifecycle v1.0.24) | Wire format `monocle-v1:<64-hex>`; constant-time comparison | integration-test | fuzz |
| VP-AUTH-002 | BC-AUTH-002 (SS-daemon-lifecycle v1.0.24) | Two-body taxonomy: absent header → `missing_auth_token`; any value-present failure → `invalid_auth_token` (collapsed) | integration-test | fuzz |
| VP-LOCK-001 | BC-LOCK-001 (SS-daemon-lifecycle v1.0.24) | Lock-file `contract_version: 1` first key; readers gate on field | integration-test | mutation-test |
...
compile-time-check probes). Cross-reference: PRD v1.24 §7 RTM Test Type
...
**Traces to:** BC-DAEMON-001 (PRD v1.24 §BC-DAEMON-001; SS-daemon-lifecycle.md
v1.0.24 §Health and Status Endpoints).
...
are cargo integration tests per Rust convention; PRD v1.24 §7 RTM Test
...
7. **Semver-regex format for `version` field** (per BC-DAEMON-001
   ...
   v1.24 commit a71ca67 via I-R91-3 HIGH closure):
   ...
   v1.24 (the v1.26 burst was authored against PRD v1.19 commit
   ...
   v1.24 commit a71ca67 — pin propagation only, no BC content change;
   the constraint reference is current per PRD v1.24).
...
v1.24 §BC-DAEMON-001, Verification subsection).

---

### §VP-DAEMON-002 — `/status` Authenticated Daemon-State JSON with 10 Required Fields

**Traces to:** BC-DAEMON-002 (PRD v1.24 §BC-DAEMON-002; SS-daemon-lifecycle.md
v1.0.24 §Health and Status Endpoints).
...
PRD v1.24 §7 RTM Test Type column labels this BC `Integration`).
...
7. **Numeric-type and range probes (F-R89-4 closure — probe-matrix
   ...
   constraint lifted in F-R91 / PRD v1.24 commit a71ca67 via I-R91-3
...
(per PRD v1.24 §BC-DAEMON-002, Verification subsection).

---

### §VP-DAEMON-003 — 256 KiB Body Limit; HTTP 413 on Excess

**Traces to:** BC-DAEMON-003 (PRD v1.24 §BC-DAEMON-003; SS-daemon-lifecycle.md
v1.0.24 §Body Size Limit).
...
are cargo integration tests; PRD v1.24 §7 RTM Test Type column labels
...
v1.24 §BC-DAEMON-003, Verification subsection).

---

### §VP-DAEMON-004 — 10-Second Graceful Shutdown Drain

**Traces to:** BC-DAEMON-004 (PRD v1.24 §BC-DAEMON-004; SS-daemon-lifecycle.md
v1.0.24 §Daemon Lifecycle Protocol §Shutdown Signal Handling, §Drain, and §Hard Shutdown).
...
6. **Exit-code taxonomy (5-code POSIX 128+N per PRD v1.24 §BC-DAEMON-004
...
- `tokio::signal::unix::signal(SignalKind::terminate())` is the SIGTERM
  ...
  selection (per arch v1.0.24 §Hard Shutdown step 6d).
...
6. **5-code POSIX exit taxonomy probe matrix (per PRD v1.24 §BC-DAEMON-004
...
(exit-code 5-code POSIX taxonomy probes per PRD v1.24 §BC-DAEMON-004
canonical test vectors).

**Test names:**
- `test_BC_DAEMON_004_graceful_shutdown_503_on_new_requests` (per PRD
  v1.24 §BC-DAEMON-004, Verification subsection — primary HTTP 503 +
  `Retry-After: 10` + body taxonomy assertion).
- `test_BC_DAEMON_004_exit_codes_posix_distinct` (per PRD v1.24
...
**Traces to:** BC-DAEMON-005 (PRD v1.24 §BC-DAEMON-005; SS-daemon-lifecycle.md
v1.0.24 §Daemon Lifecycle Protocol §Start Sequence and §Hard Shutdown;
...
forward verbatim into PRD v1.24 commit a71ca67 per C-R90-1 PO PRD-pin
propagation sweep).

**Mechanical property:**

0. **Runtime-dir resolution chain (per PRD v1.24 §BC-DAEMON-005 precondition
   2 + arch v1.0.24 §Start Sequence step 1; F-R70-1 closure):** before any
...
7. **Asymmetry with BC-ENGINE-002-ERR `HomeUnresolvable` is intentional**
   (per PRD v1.24 §BC-DAEMON-005 invariant 4 + arch v1.0.24 §Start
...
`<crate>/tests/` are cargo integration tests; PRD v1.24 §7 RTM Test
...
4. Daemon graceful shutdown via synthetic SIGTERM → after drain
   ...
   post-drain).
5. **4-path resolution chain probe matrix (per PRD v1.24 §BC-DAEMON-005
...
9. **Runtime-dir mode 0o700 owner-only enforcement (per PRD v1.24
   §BC-DAEMON-005 Postcondition 8 + EC-052 + arch v1.0.24 §Start
   Sequence step 1; F-R75-1 VP-side closure + F-R79-3 PRD-side
   lift_invariants_to_bcs closure that elevated the 0o700 contract
   from EC-052 to §Postcondition tier in PRD v1.24 commit a71ca67
   (PRD pin now bumped to v1.24 commit a71ca67 per F-R94 PO PRD-pin
...
v1.24 §BC-DAEMON-005, Verification subsection — covers the lock-file
mode/lifecycle assertions AND the EC-057/058/059 resolution-chain
probes via the canonical test-vector matrix in PRD v1.24 §BC-DAEMON-005
...
**Traces to:** BC-DAEMON-006 (PRD v1.24 §BC-DAEMON-006; SS-daemon-lifecycle.md
v1.0.24 §Daemon Lifecycle Protocol §Crash Recovery).
...
10. **Enum-value-set for `shutdown_reason`:** `shutdown_reason` MUST be a
    ...
    enumerates the 3-element `shutdown_reason` closed-set in PRD v1.24
    commit a71ca67 (PRD line 419) — `closed-set enum — exactly one of
    ...
    Invariant. R91 PO closure burst (commit a71ca67) lifted the four
...
(per PRD v1.24 §BC-DAEMON-006, Verification subsection).
...
location; PRD v1.24 §7 RTM Test Type column labels this BC
...
4. **Absence-of-field for `None` Options (BC-RING-001 EC-001 normative
   ...
   on `tool_name` and `tool_input` (per arch v1.0.24 commit 58af8de
   `HookEventRecord` struct definition + SessionStart None-case
   serialization example; PRD v1.24 §BC-RING-001 EC-001 normative
...
**Test name:** `test_BC_RING_001_format_version_first_key` (per PRD v1.24
...
(per PRD v1.24 §BC-AUTH-001, Verification subsection).

---

### §VP-AUTH-002 — Auth Header Two-Body Taxonomy: `missing_auth_token` vs `invalid_auth_token`

**Traces to:** BC-AUTH-002 (PRD v1.24 §BC-AUTH-002; SS-daemon-lifecycle.md
v1.0.24 §Daemon Lifecycle Protocol §Start Sequence; architect adjudication
...
(per PRD v1.24 §BC-AUTH-002, Verification subsection).
...
**Test name:** `test_BC_LOCK_001_contract_version_first_key` (per PRD v1.24
...
PRD v1.24 §BC-ABI-001, Verification subsection).
...
verification driver; PRD v1.24 §7 RTM Test Type column labels this BC
`Lint/compile`).
...
**Test name:** `test_BC_ABI_002_abi_version_const_exported` (per PRD v1.24
...
file and walking all `Item::Enum` nodes; per PRD v1.24 §BC-TYPES-001
invariant 1; PRD v1.24 §7 RTM Test Type column labels this BC
...
v1.24 §BC-TYPES-001, Verification subsection).
...
`monocle-core/tests/factory_trait_surface.rs`; PRD v1.24 §7 RTM Test
...
(per PRD v1.24 §BC-FACTORY-001, Verification subsection).
...
7. **Empty-string `current_cycle` fixture (per BC-FACTORY-002 §Edge
   ...
   verified post-R91: EC-061 was added to PRD v1.24 §BC-FACTORY-002
...
(per PRD v1.24 §BC-FACTORY-002, Verification subsection).
...
v1.24 §BC-PROTO-001a, Verification subsection).
...
cargo integration tests; PRD v1.24 §7 RTM Test Type column labels this
...
**Test name:** `test_BC_PROTO_001b_schema_version_rust_field` (per PRD v1.24
...
**Traces to:** BC-PROTO-002 (PRD v1.24 §BC-PROTO-002; SS-core-types-and-abi.md
...
architecture artifact specifies a Phase 1 dispatcher; PRD v1.24
...
- **Phase 1:** integration-test (structural — cross-property recap of
  ...
  `monocle-proto/tests/schema_version.rs` per PRD v1.24 §7 RTM Test Type
...
PRD v1.24 §BC-PROTO-002 (Phase 4 test name `test_BC_PROTO_002_schema_version_validation_skip_unknown`
documented in PRD v1.24 §BC-PROTO-002 Verification subsection for Phase 4
...
`monocle-core/tests/engine_module_surface.rs`; PRD v1.24 §7 RTM Test
Type column labels this BC `AST audit (syn 2)`).
...
(per PRD v1.24 §BC-ENGINE-001, Verification subsection).
...
(per PRD v1.24 §BC-ENGINE-002, Verification subsection).
...
files in `<crate>/tests/` are cargo integration tests; PRD v1.24 §7
...
(per PRD v1.24 §BC-ENGINE-002-ERR, Verification subsection).
...
(per PRD v1.24 §BC-ENGINE-003, Verification subsection — hybrid name
...
|-------|----------------|-------|---------------------|-------------------|
| BC-DAEMON-001 | PRD v1.24 / SS-daemon-lifecycle.md v1.0.24 | VP-DAEMON-001 | integration-test | `monocle-runtime/tests/healthz_endpoint.rs` |
| BC-DAEMON-002 | PRD v1.24 / SS-daemon-lifecycle.md v1.0.24 | VP-DAEMON-002 | integration-test | `monocle-runtime/tests/status_endpoint_auth.rs` |
| BC-DAEMON-003 | PRD v1.24 / SS-daemon-lifecycle.md v1.0.24 | VP-DAEMON-003 | integration-test | `monocle-runtime/tests/body_size_limit.rs` |
| BC-DAEMON-004 | PRD v1.24 / SS-daemon-lifecycle.md v1.0.24 | VP-DAEMON-004 | integration-test | `monocle-runtime/tests/graceful_shutdown.rs` + `monocle-runtime/tests/daemon_lifecycle.rs` |
| BC-DAEMON-005 | PRD v1.24 / SS-daemon-lifecycle.md v1.0.24 | VP-DAEMON-005 | integration-test | `monocle-runtime/tests/lock_file_lifecycle.rs` |
| BC-DAEMON-006 | PRD v1.24 / SS-daemon-lifecycle.md v1.0.24 | VP-DAEMON-006 | integration-test | `monocle-runtime/tests/crash_recovery.rs` |
| BC-RING-001 | SS-daemon-lifecycle.md v1.0.24 | VP-RING-001 | integration-test | `monocle-runtime/tests/jsonl_ring.rs` |
| BC-AUTH-001 | SS-daemon-lifecycle.md v1.0.24 | VP-AUTH-001 | integration-test | `monocle-runtime/tests/auth_token_lifecycle.rs` |
| BC-AUTH-002 | SS-daemon-lifecycle.md v1.0.24 | VP-AUTH-002 | integration-test | `monocle-runtime/tests/auth_header_rejection.rs` |
| BC-LOCK-001 | SS-daemon-lifecycle.md v1.0.24 | VP-LOCK-001 | integration-test | `monocle-runtime/tests/lock_file_contract.rs` |
...
- **NFR-001 — hook latency p99 ≤ 300 ms (`PreToolUse`, `Stop`,
  `SessionStart`, `UserPromptSubmit` hook surfaces per PRD v1.24
  §NFR-001).** Per brief §Success Criteria latency target and PRD v1.24
...
- **NFR-002 — hook latency p99 ≤ 2000 ms (`Notification` hook surface,
  per PRD v1.24 §NFR-002 — scoped to `/hooks/notification` only;
...
- **NFR-003 — permission overlay first-paint ≤ 100 ms.** Per PRD v1.24
...
reference; it is NOT a Phase 1 monocle BC in PRD v1.24 §2.1 or §7 RTM
...
- **NFR-006 — bounded event bus with visible drop counter; 1000
  ...
  status bar.** Per PRD v1.24 §NFR-006 specification (PRD line 1208);
...
`2026-05-17T03:30:00Z`.

1. `.factory/specs/prd.md` v1.24 (commit a71ca67) — canonical BC source
   ...
   adjudications; PRD v1.24 is the F-R100 Burst 3 pin-propagation
   closure (commit a71ca67) — arch v1.0.23 → v1.0.24 (commit d088123
   → 58af8de) pin propagation per Extension 15 + SE-15e mandatory
   cascade after F-R100 Burst 2 architect closure (commit 58af8de);
   SE-17g (33rd discipline) second application; no PRD normative
   content change at v1.24; 22 BCs unchanged at v1.24; edge-case
   count unchanged at 61 (PG-5 historical-predecessor: PRD v1.23 was
...
2. `.factory/specs/architecture/SS-daemon-lifecycle.md` v1.0.24 (commit
   58af8de) — source of BC-DAEMON-001..006, BC-RING-001, BC-AUTH-001,
   ...
   EC-001 normative absence-of-field form (PRD v1.24 §BC-RING-001
   ...
   only.
   F-R100 Burst 2 closure landed in arch v1.0.24 commit 58af8de —
   current canonical arch source per F-R100 Burst 2 closure chain —
   ...
   Default); manifest unchanged at v1.1.15 in this burst (arch-only
   bump); no BC content lift in v1.0.24 — F-R100 Burst 2 is §Trace
   evidence-fidelity correctness + SE-17g first-application only).
...
6. `.factory/specs/architecture/SS-deps-pin-manifest.md` v1.1.15 (commit
   ...
   (arch v1.0.23 → v1.0.24 commit d088123 → 58af8de); manifest held
