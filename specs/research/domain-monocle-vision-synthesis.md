...
# Monocle Vision Synthesis (v2.2.3 — I25-001 SessionState Lifecycle Consistency, Approved)
...
| **2.2.3** | **2026-06-13** | **product-owner** | **I25-001 consistency correction — `SessionState` code block and LAUNCH prose lifecycle enumeration aligned to the ratified canonical enum (`SS-session-manager.md` / ADR-0009 / BC-2.08.001 v1.3.1 <!-- version-pin-historical: BC version at time of I25-001 authoring -->); no decision change.** (1) §SessionManager Rust code block: `Created` (removed — spawn_session() goes directly to Launching; never persisted or observed) and `Killed` (removed — superseded by `Terminating`) removed from the enum; `Terminating` added with description. (2) §v1A LAUNCH prose: `Created → Launching → Running → Detached → Terminated` → `Launching → Running → Detached → Terminating → Terminated`. This is a consistency correction only — the ratified enum was already correct at the BC/SS/ADR layer; the old code block and prose string were survivng stale L1 representations. Status: APPROVED (consistency-only; no scope change). |
...
/// Canonical definition: SS-session-manager.md (SS-08). I25-001 alignment (2026-06-13).
///   Created — REMOVED: spawn_session() transitions directly to Launching.
///   Killed   — REMOVED: superseded by Terminating (observable in-flight kill status).
...
pub enum SessionState {
    ...
    Terminating,  // kill_session() called; DaemonToHost::Kill sent; awaiting
                  // session-host confirmation (StateChanged::Terminated)
...
**v2.2.3 (2026-06-13):** I25-001 consistency correction — `SessionState` code block (§SessionManager) and LAUNCH prose lifecycle enumeration (§v1A Launch Wave) aligned to the ratified canonical enum (SS-session-manager.md / ADR-0009 / BC-2.08.001 v1.3.1 <!-- version-pin-historical: BC version at time of I25-001 authoring -->). Two stale hits fixed: (1) §SessionManager Rust code block retained `Created` and `Killed` as enum variants — both REMOVED from the ratified enum (`Created` was never persisted or observed; `Killed` was superseded by `Terminating` to provide observable in-flight kill status); `Terminating` added with description. (2) §v1A LAUNCH prose: `Created → Launching → Running → Detached → Terminated` → `Launching → Running → Detached → Terminating → Terminated`. No decision change — the ratified enum was already correct at the BC/SS/ADR layer; this propagates those decisions to L1 prose and the embedded code example. Status: APPROVED (consistency-only; no scope change).
