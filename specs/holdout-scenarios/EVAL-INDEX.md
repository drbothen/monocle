...
inputs:
  ...
  - {path: .factory/stories/S-036-session-manager-rediscovery.md, version: "1.6"}
...
**Bump:** 1.42 → 1.43.
**Scope:** `inputs[]` S-046 story pin updated: `"1.8"` → `"1.9"` (S-046 bumped to v1.9: BC-2.05.009 inputs pin v1.5.5→v1.5.11, BC-2.05.011 inputs pin v1.2.5→v1.2.11, SS-session-manager inputs pin v2.6.1→v2.17.1; body accuracy verified — §Daemon-Side Per-Client Fan-out Channel contract stable, spawn_session/DaemonState integration claims confirmed stable). No holdout scenario IDs, BCs, or behavioral semantics changed.
**SE-16d PASS:** 2026-06-22 >= 2026-06-22 (v1.42 last bump date). PASS (same-day sequential patch).

## §Trace v1.38 — POL-11 pin cascade: S-040 input pin v1.7 → v1.8 (2026-06-21)

**Bump:** 1.37 → 1.38.
**Scope:** `inputs[]` S-040 story pin updated: `"1.7"` → `"1.8"` (S-040 bumped to v1.8: BC-2.09.002 input pin v1.2.0→v1.2.2, BC-2.09.004 input pin v1.0.9→v1.0.11, SS-embedded-pty v1.13.0→v1.14.0; ADV-MED-001 Esc Press-only kind guard task + test added; ADV-LOW-001+LOW-002 stale doc-comment correction task added). No holdout scenario IDs, BCs, or behavioral semantics changed.
**SE-16d PASS:** 2026-06-21 >= 2026-06-21 (v1.37 last bump date). PASS (same-day sequential patch).

## §Trace v1.37 — POL-11 pin cascade: S-040 input pin v1.6 → v1.7 (2026-06-21)

**Bump:** 1.36 → 1.37.
**Scope:** `inputs[]` S-040 story pin updated: `"1.6"` → `"1.7"` (S-040 bumped to v1.7: BC-2.09.005 input pin v1.0.6→v1.0.7 (EC-245 over-ceiling paste guard); ADV-HIGH-002 paste-ceiling task + ADV-HIGH-001 modified-arrow exact-equality task added). No holdout scenario IDs, BCs, or behavioral semantics changed.
**SE-16d PASS:** 2026-06-21 >= 2026-06-21 (v1.36 last bump date). PASS (same-day sequential patch).
...
**Bump:** 1.34 → 1.35.
**Scope:** `inputs[]` S-040 story pin updated: `"1.5"` → `"1.6"` (S-040 bumped to v1.6: input pins finalized to BC-2.09.002 v1.2.0 and BC-2.09.005 v1.0.6 after PO BC bumps; AC-015/EC-218 Esc+modifier coverage added). No holdout scenario IDs, BCs, or behavioral semantics changed.
**SE-16d PASS:** 2026-06-21 >= 2026-06-21 (v1.34 last bump date). PASS (same-day sequential patch).
...
**Bump:** 1.32 → 1.33.
**Scope:** `inputs[]` S-040 story pin updated: `"1.3"` → `"1.4"` (S-040 bumped to v1.4 to add architect-specified implementation Tasks for Kitty CSI-u redesign; input pins updated for BC-2.09.002 v1.1.8, BC-2.09.004 v1.0.8). No holdout scenario IDs, BCs, or behavioral semantics changed. EVAL-INDEX is an active INDEX document per ADR-0007 §Active-set rules; its input pins must track canonical current versions.
**SE-16d PASS:** 2026-06-21 >= 2026-06-20 (v1.32 last bump date). PASS.

## §Trace v1.30 — POL-11 pin cascade: S-039 input pin v1.3 → v1.8 (2026-06-20)

**Bump:** 1.29 → 1.30.
**Scope:** `inputs[]` S-039 story pin updated: `"1.3"` → `"1.8"` (S-039 was at v1.8 after adversarial convergence completed; EVAL-INDEX input pin was stale at v1.3). No holdout scenario IDs, BCs, or behavioral semantics changed. EVAL-INDEX is an active INDEX document per ADR-0007 §Active-set rules; its input pins must track canonical current versions.
**SE-16d PASS:** 2026-06-20 >= 2026-06-20 (same day as v1.29). PASS.
