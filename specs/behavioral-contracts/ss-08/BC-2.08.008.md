...
3. `SessionStateChanged` is enqueued BEFORE `SessionListUpdate` into each client's per-client
   ...
   See SS-daemon-wiring-v2-delta.md v1.11.3 §3b for the canonical emission code pattern.
...
4a. **`rename_session()` does NOT emit `SessionStateChanged`.** Rename updates `display_name`
   ...
   See SS-daemon-wiring-v2-delta.md v1.11.3 §3b emission table.
...
| Architecture Source | SS-session-manager.md v2.6.0 §Session lifecycle state machine (state transitions, including re-discovery GC and Detached re-discovery; IPC handler generates UUID + sends SpawnAck before spawn_session()); SS-embedded-pty.md v1.6.0 §TUI AppMode Extensions (SessionCreation::Launching auto-transition to EmbeddedTerminal; `launching_session_id: Option<String>` field added — F-P41-IMP-001); SS-ipc.md v1.23.2 §ServerToClient::SpawnAck (new variant; per-client point-to-point delivery before SessionStateChanged{Launching}); SS-daemon-wiring-v2-delta.md v1.11.3 §3b (SessionStateChanged emission rule, ordered-pair-split-on-Full disconnect rule, rename-only-SessionListUpdate rule) |
