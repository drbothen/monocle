...
2. For each sidecar with accepted `schema_version` (1, 2, or 3):
   ...
   b. If alive: apply state-dependent handling:
      - **State `Launching` or `Running`:** Verify SO_PEERCRED peer uid matches daemon uid
        (per SS-session-manager.md v2.5.1 §Per-session UDS security I5); if mismatch → log
        ...
        RETIRED per SS-session-manager.md v2.5.1); on `ScrollbackDumpComplete` receipt, register
...
| Architecture Source | SS-session-manager.md v2.5.1 §Daemon startup: session re-discovery (including §Re-discovery state handling — I4 all states covered; I3-002 Terminating watchdog; I3-005 Detached preservation); §session-state.json schema (schema_version history 1/2/3); SS-daemon-wiring-v2-delta.md v1.11.2 §daemon_start_sequence() — session re-discovery step (step 8b placement and insertion invariant) |
