...
| Architecture Source | SS-session-manager.md v2.16.0 §Daemon startup: session re-discovery; SS-session-manager.md v2.16.0 §monocle-session-host binary §startup sequence step 2; ADR-0009 v1.0.2 §Decision |
...
- SE-16d monotonicity: v1.2.7 > v1.2.6. PASS.
...
- SE-16d monotonicity: v1.2.4 timestamp 2026-06-19 >= v1.2.3 timestamp auto. PASS.
...
- Architecture Source: two violations fixed:
  1. Second `SS-session-manager.md §monocle-session-host binary §startup sequence step 2` (no version pin) → `SS-session-manager.md v1.8.1 §monocle-session-host binary §startup sequence step 2`. Pin-symmetry rule: all refs in a multi-doc cell must carry explicit version pins. <!-- version-pin-historical: §Trace I17-001 fix record; v1.8.1 is SS-session-manager at Pass-17 fix time -->
