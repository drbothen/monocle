...
4. GC is managed by a single tokio task per session that fires via `tokio::time::sleep`.
   ...
   `session_error_to_code()` (SS-session-manager.md v2.8.0 §Terminated-in-grace defensive
...
| Architecture Source | SS-session-manager.md v2.8.0 §Session GC policy; SS-session-manager.md v2.8.0 §Terminated-in-grace defensive action×state matrix (F-P52-001); SS-session-manager.md v2.8.0 §session_error_to_code() (InvalidSessionName → "rename_failed"); SS-ipc.md v1.24.0 §ServerToClient::Error taxonomy; SS-daemon-wiring-v2-delta.md v1.11.4 |
