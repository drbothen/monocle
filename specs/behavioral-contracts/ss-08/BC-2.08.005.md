...
4. GC is managed by a single tokio task per session that fires via `tokio::time::sleep`.
   ...
   `session_error_to_code()` (SS-session-manager.md §Terminated-in-grace defensive
...
| Architecture Source | SS-session-manager.md v2.17.1 §Session GC policy; SS-session-manager.md v2.17.1 §Terminated-in-grace defensive action×state matrix (F-P52-001); SS-session-manager.md v2.17.1 §session_error_to_code() (InvalidSessionName → "rename_failed"); SS-ipc.md v1.24.0 §ServerToClient::Error taxonomy; SS-daemon-wiring-v2-delta.md v1.12.0 |
...
- SE-16d monotonicity: 1.0.12 > 1.0.11. PASS.
...
- SE-16d monotonicity: v1.0.10 > v1.0.9. PASS.
...
- Architecture Source pin cascade: SS-session-manager.md v2.14.0 → v2.15.0; SS-daemon-wiring-v2-delta v1.11.4 → v1.12.0 (single-writer mandate). No behavioral content changes to this BC.
- SE-16d monotonicity: v1.0.8 timestamp 2026-06-19 >= v1.0.7 timestamp 2026-06-19. PASS.
...
- SE-16d monotonicity: v1.0.7 timestamp 2026-06-19 >= v1.0.6 timestamp auto. PASS.
