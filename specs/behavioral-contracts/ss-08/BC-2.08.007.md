...
2. The proxy task for this session is terminated (`proxy_task.take().map(|t| t.abort())`). `proxy_task` is typed `Option<JoinHandle<()>>` as of SS-session-manager.md v2.10.0; `.take()` clears the field and `.map(|t| t.abort())` aborts the task if present.
...
3. The scrollback is transferred as `HostToDaemon::ScrollbackChunk*` + `HostToDaemon::
   ...
   live parser state. See SS-session-manager.md v2.10.0 §Screen-state transfer for the
