...
2. The proxy task for this session is terminated (`proxy_task.take().map(|t| t.abort())`). `proxy_task` is typed `Option<JoinHandle<()>>` <!-- version-pin-historical: SS-session-manager v2.15.1 at BC-2.08.007 authoring time -->; `.take()` clears the field and `.map(|t| t.abort())` aborts the task if present.
...
3. The scrollback is transferred as `HostToDaemon::ScrollbackChunk*` + `HostToDaemon::
   ...
   live parser state. See SS-session-manager.md §Screen-state transfer for the
...
5. SO_PEERCRED peer-credential check is mandatory on attach before sending any messages.
   The four distinct attach-failure dispositions for a `Detached` session are:
   - **EC-187 ConnectFailed** (socket not present / connect returns error): host confirmed dead
     via `kill(pid, None)` probe → `transition_to_terminated_standalone` (publishing
     `SessionStateChanged{Terminated}` before `SessionListUpdate`, spawning GC) →
     `Err(SessionHostDead)`. Rationale: host process is dead.
   - **PeerCredFailed** (SO_PEERCRED uid mismatch): `transition_to_terminated_standalone`
     (same broadcasts + GC) → `Err(SessionHostDead)`. Rationale: uid mismatch is structural —
     the socket is owned by a process that is not our session-host child; the session is
     effectively dead. Leaving it `Detached` would allow infinite retry against a hostile or
     dead socket.
   - **ProtocolError** (unexpected message type or framing error AFTER uid match succeeds):
     stay `Detached` (no transition, no broadcast) → `Err(SessionHostDead)`. Rationale:
     SO_PEERCRED passed means the peer uid matched — the host process is alive and is ours.
     The error is in the attach exchange, not in host liveness. A subsequent `attach_session()`
     is a legitimate retry.
   - **EC-188 Timeout** (5s elapsed without `ScrollbackDumpComplete`): SIGTERM sent to
     session-host PID → `transition_to_terminated_standalone` (publishing
     `SessionStateChanged{Terminated}` before `SessionListUpdate`, spawning GC) →
     `Err(SessionHostDead)`. Rationale: SIGTERM declares the host non-responsive and actively
     kills it. Leaving it `Detached` would create a re-attachable entry for a process being
     killed — inconsistent with EC-187/PeerCredFailed and semantically incorrect (the entry
     would report "Detached" for a host we are actively terminating).
   This enumeration is the canonical and complete set of attach-failure dispositions. No
   additional failure subpaths exist for `attach_session()` on a `Detached` entry.
...
| EC-188 | `ScrollbackDumpComplete` (full `ScrollbackChunk*` + `ScrollbackDumpComplete` sequence) not received within 5s | Session treated as non-responsive (Invariant 4); SIGTERM sent to session-host PID (same non-responsive handling as re-discovery 5s timeout in BC-2.08.004); `SessionEntry.state` → `Terminated` via `transition_to_terminated_standalone` (publishing `SessionStateChanged{Terminated}` before `SessionListUpdate`, spawning GC); `attach_session()` returns `Err(SessionError::SessionHostDead { session_id })`; maps to `"attach_failed"` via `session_error_to_code(IpcOp::Attach, SessionHostDead)` per SS-session-manager.md §session_error_to_code attach-path arm. Rationale: SIGTERM declares the host non-responsive and actively kills it — leaving the entry `Detached` would create a re-attachable entry for a process being killed, inconsistent with EC-187 (ConnectFailed → Terminated) and PeerCredFailed (→ Terminated). The retired single-message `ScrollbackDump` form is NOT accepted — only the chunked protocol terminating with `ScrollbackDumpComplete`. `AttachTimeout` does not exist in the canonical `SessionError` taxonomy. |
...
- SE-16d monotonicity: 1.5.9 > 1.5.8. PASS.
...
- Architecture Source pin updated: SS-session-manager.md v2.15.1 → v2.16.0. Inline prose "v2.15.1"
  references converted to version-free anchor form (section is unchanged in v2.16.0). No behavioral
  content changes to this BC.
- SE-16d monotonicity: v1.5.8 > v1.5.7. PASS.
...
**F-S035-PASS5-MED-001 adjudication — EC-188 timeout → Terminated via transition_to_terminated_standalone; Invariant 5 enumerates all four attach-failure dispositions** (2026-06-19T00:00:00Z):

- **Finding (F-S035-PASS5-MED-001):** EC-188 (5s timeout: ScrollbackDumpComplete not received)
  SIGTERMs the session-host PID but performed NO state transition — the entry + sidecar stayed
  `Detached`, leaving a re-attachable entry referencing a host being actively killed. This was
  inconsistent with EC-187 (ConnectFailed → Terminated) and PeerCredFailed (→ Terminated).
  The EC-188 test docstring stated "Transition the session to Terminated" but the test body
  never asserted it, and the prior spec text did not require it.

- **Decision: A — EC-188 timeout → `transition_to_terminated_standalone`.**
  Rationale: SIGTERM declares the host non-responsive and actively kills it. A Detached
  re-attachable entry for a SIGTERM'd host is semantically incorrect — the entry claims
  "Detached" (re-attachable) while the host is being terminated. EC-187 ConnectFailed
  transitions to Terminated on less certainty of host-death (connect failed, no active kill);
  EC-188 has stronger certainty (we actively sent SIGTERM) and must therefore also transition.
  Decision B (stay Detached) was rejected because the SIGTERM constitutes a declaration of
  host-death; leaving a re-attachable entry for a SIGTERM'd host has no valid use case.
  Possible SIGTERM survival (catchable signal): our session-host implementation does not
  catch and ignore SIGTERM; background watchdog/GC handles any stale entries; the correct
  behaviour for a subsequent attach on Terminated is Err(SessionHostDead), not a silent retry.

- **Changes (normative):**
  - **EC-188** — updated to state `SessionEntry.state → Terminated` via
    `transition_to_terminated_standalone` (publishing `SessionStateChanged{Terminated}` before
    `SessionListUpdate`, spawning GC), with rationale.
  - **Invariant 5** — rewritten to enumerate all four attach-failure dispositions explicitly:
    EC-187 ConnectFailed → Terminated; PeerCredFailed → Terminated; ProtocolError → stays
    Detached; EC-188 Timeout → Terminated. This enumeration is canonical and complete.
  - **Architecture Source** — pin updated to SS-session-manager.md v2.14.0 (EC-188 timeout
    subpath added to Detached cell of action×state matrix).

- **SE-16d monotonicity:** v1.5.5 timestamp 2026-06-19 >= v1.5.4 timestamp 2026-06-19. PASS.
