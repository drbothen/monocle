...
2. The TUI has received `ServerToClient::InitialState` or `ServerToClient::SessionListUpdate`
   with session data from the daemon as `Vec<SessionSnapshot>` (SS-ipc.md v1.22.0 — the wire
...
1. The sessions panel renders a grouped list from `Vec<SessionSnapshot>` received via
   `InitialState.sessions` or `SessionListUpdate.sessions` (SS-ipc.md v1.22.0 `SessionSnapshot`
...
4. Monocle-launched sessions show a `[M]` badge. Externally-detected sessions show `[E]`.
   ...
   (SS-ipc.md v1.22.0 `SessionSnapshot` type): `Some(true)` → `[M]`, `Some(false)` → `[E]`,
...
| Architecture Source | SS-ipc.md v1.22.0 §SessionSnapshot (wire boundary type; `degraded` and `degraded_reason` fields; `spawned_by_monocle: Option<bool>` field); SS-session-manager.md v2.4.0 §SessionManager §Public API (session_list() returns Vec<SessionSnapshot>); SS-embedded-pty.md v1.6.0 §Fast switching; SS-session-manager.md v2.4.0 §monocle-session-host startup sequence §I3-009 (degraded-env surfaced via HostToDaemon::StateChanged.degraded_env + SessionSnapshot.degraded) |
