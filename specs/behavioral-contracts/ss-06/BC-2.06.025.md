...
2. The TUI has received `ServerToClient::InitialState` or `ServerToClient::SessionListUpdate`
   with session data from the daemon as `Vec<SessionSnapshot>` (SS-ipc.md — the wire
...
1. The sessions panel renders a grouped list from `Vec<SessionSnapshot>` received via
   `InitialState.sessions` or `SessionListUpdate.sessions` (SS-ipc.md `SessionSnapshot`
...
4. Monocle-launched sessions show a `[M]` badge. Externally-detected sessions show `[E]`.
   ...
   (SS-ipc.md `SessionSnapshot` type): `Some(true)` → `[M]`, `Some(false)` → `[E]`,
...
6. **Terminated-in-grace lifecycle action invariant (F-P52-001):** Sessions with
   ...
   - **Detach (`D`) on Terminated**: BLOCKED at TUI. No `ClientToServer::DetachSession` IPC
     ...
     returns idempotent Ok(()) per SS-session-manager.md §Terminated-in-grace defensive
   ...
   - **Rename (`r`) on Terminated**: BLOCKED at TUI. No `ClientToServer::RenameSession` IPC
     ...
     `"rename_failed"` per SS-session-manager.md §Terminated-in-grace defensive
   ...
   cancellable); SS-session-manager.md §Terminated-in-grace defensive action×state
   matrix (F-P52-001).
...
- SE-16d monotonicity: 1.5.12 > 1.5.11. PASS.
...
- SE-16d monotonicity: v1.5.10 > v1.5.9. PASS.
...
- SE-16d monotonicity: v1.5.9 timestamp >= v1.5.8. PASS.
...
- SE-16d monotonicity: v1.5.5 timestamp 2026-06-19 >= v1.5.4 timestamp auto. PASS.
...
- **Gap closed (F-P52-001):** BC-2.06.025 Invariant 4 (as written through v1.4.0) blocked lifecycle
  ...
  cases in SS-session-manager.md v2.7.3 §Terminated-in-grace defensive action×state matrix
...
- **Traceability Cross-Ref expanded:** Added BC-2.08.003 Invariant 2 (kill idempotency at daemon),
  ...
  SS-session-manager.md v2.7.3 §Terminated-in-grace defensive action×state matrix (F-P52-001).
