...
3. No IPC message is sent to the daemon on rotation. The rotation is a local TUI state
   change only. The daemon's `queued_prompts` is not modified.
...
| Architecture Source | SS-tui.md v1.0.0 §AppMode State Machine §Transition Function Contract (OverlayCycleNext arm); §Permission Overlay §Overlay Stack Lifecycle step 2 |
...
- Reads: SS-tui.md v1.0.0 §AppMode State Machine (OverlayCycleNext transition arm),
