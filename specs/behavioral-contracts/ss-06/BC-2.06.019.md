...
daemon's bounded event bus, as delivered in each `IpcServerMessage::StateUpdate` push from
...
1. The TUI is connected to the daemon and receiving `StateUpdate` IPC messages.
2. Each `IpcServerMessage::StateUpdate` contains a `drop_counter: u64` field that is the
...
4. The `App` struct holds `drop_counter: u64` updated from the last received `StateUpdate`.
...
3. **Counter is cumulative from daemon start:** The `drop_counter` is never reset to 0
   ...
   the counter resets to 0 in the new daemon's `StateUpdate` — the TUI reflects the
...
4. **1000 events/sec load test criterion:** Under synthetic 1000 events/sec load on
   ...
   - The drop counter is propagated via IPC `StateUpdate` (not silently discarded)
   - The TUI renders the counter when non-zero

5. **Counter renders on every draw tick:** The `drop_counter` value from the last
   `StateUpdate` is rendered on every `draw()` call while the daemon is connected. There
...
| EC-120 | Two consecutive `StateUpdate` messages with same `drop_counter` value | No visual change; renders `drops: N` (or blank if 0) same as before; no flicker |
| EC-121 | `drop_counter` decreases between two `StateUpdate` messages (e.g., daemon restart with new counter starting at 0) | TUI updates to the new counter value; if new value is 0, widget disappears; this is correct — the new daemon has a fresh counter |
...
| EC-124 | `StateUpdate` messages arrive at 60fps (every 16ms) | Each `drop_counter` update is rendered on the next draw tick; no message is skipped in rendering (the draw loop drains IPC before drawing) |

## Canonical Test Vectors

| Input | Expected Output | Category |
|-------|----------------|----------|
| `StateUpdate { drop_counter: 0 }` | Status bar shows no drop counter text | happy-path |
| `StateUpdate { drop_counter: 1 }` | Status bar shows `drops: 1` in yellow | happy-path |
| `StateUpdate { drop_counter: 42 }` | Status bar shows `drops: 42` in yellow | happy-path |
| `StateUpdate { drop_counter: 0 }` after `drop_counter: 42` (daemon restart) | `drops: 42` disappears; status bar shows no counter | edge-case |
...
| Architecture Module | monocle-tui (status bar renderer, `App::drop_counter: u64`); monocle-ipc (IPC `StateUpdate` message type) per ARCH-INDEX SS-06 |
| Architecture Source | SS-tui.md v1.0.0 §Panel Architecture §Status Bar (drop counter subsection: "receives drop_counter: u64... renders as `drops: N` when N > 0; renders nothing when N == 0") |
...
- Reads: SS-tui.md v1.0.0 §Panel Architecture §Status Bar (drop counter subsection);
