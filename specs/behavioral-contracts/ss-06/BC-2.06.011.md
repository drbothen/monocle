...
`PromptModal` from the `VecDeque` stack. The TUI sends `IpcClientMessage::DecisionResponse
{ prompt_id, decision: Decision::AcceptOnce }` to the daemon via the bounded IPC send
...
1. **IPC send enqueued:** `IpcClientMessage::DecisionResponse { prompt_id: stack.front().prompt_id, decision: Decision::AcceptOnce }` is enqueued on `App::ipc_tx`. This is non-blocking: the TUI does not wait for the daemon to acknowledge before proceeding with the state transition.
...
1. The `prompt_id` sent in the `DecisionResponse` is the `Uuid` of the front `PromptModal`
   at the time `[1]` is pressed. If the daemon receives a `DecisionResponse` for a
   `prompt_id` it does not recognize (e.g., the prompt timed out), the daemon silently
   discards the response — the TUI does not need to handle this case.
...
| EC-080 | Daemon has already timed out the prompt (no HTTP response connection waiting) | Daemon silently discards the `DecisionResponse`; TUI is unaware; no error surfaced to user; the TUI-side pop already occurred and is correct |
...
|----------------------|----------------|----------|
| `Overlay { stack: [P1], prior: Sessions }`, `PermissionAcceptOnce` | `Dashboard { focused: Sessions }` + `DecisionResponse { prompt_id: P1.id, decision: AcceptOnce }` enqueued | happy-path |
| `Overlay { stack: [P1, P2], prior: Sessions }`, `PermissionAcceptOnce` | `Overlay { stack: [P2], prior: Sessions }` + `DecisionResponse { prompt_id: P1.id, decision: AcceptOnce }` enqueued | happy-path |
...
|--------|----------|-------------|
| VP-TBD | IPC `DecisionResponse` with correct `prompt_id` is enqueued before state transition | integration test (mock IPC channel; assert message received before mode changes) |
...
| Architecture Source | SS-tui.md v1.0.0 §Permission Overlay §Overlay Stack Lifecycle step 3 (Decide); §AppMode State Machine §Transition Function Contract (PermissionAcceptOnce arm) |
