...
2. `crossterm::event::EnableMouseCapture` was called during `App::enter_embedded_terminal()`
   ...
   (I3 fix per SS-embedded-pty.md v1.11.0: global mouse capture is NOT used; it would steal
...
2. The function returns `Some(bytes)` encoding the event in SGR mouse mode.
   The complete base-`Ps` table (per SS-embedded-pty.md v1.11.0 §mouse_event_to_pty_bytes):
...
1. Mouse capture and SGR 1006 are symmetric and SCOPED to `EmbeddedTerminal`:
   ...
   - `EnableMouseCapture` is NOT called at TUI startup; it is NOT globally active outside
     ...
     SS-embedded-pty.md v1.11.0 §I3 UX tradeoff.
...
4. The complete base-Ps enumeration: Down(L/M/R)=0/1/2 (terminator `M`); Up(L/M/R)=0/1/2
   ...
   Alt|=8, Ctrl|=16. This matches SS-embedded-pty.md v1.11.0 §mouse_event_to_pty_bytes
...
| Architecture Source | SS-embedded-pty.md v1.11.0 §Mouse support (SGR mode); §I3 UX tradeoff (scoped mouse capture) |
