...
**OBS-001 — Editorial: replace nonexistent `screen().scrollback_len()` with accurate read-back probe description** (2026-06-21):

vt100 0.16.2 does not expose a public `scrollback_len()` method on `Screen`. The method lives on the
internal `Grid` type and is not part of the public API. Two references in v1.4.0 cited this nonexistent
method as the canonical approach for determining the clamp upper bound:

1. PC-5 algorithm: the `min(parser.screen().scrollback_len(), new_offset)` clamp expression.
2. §Trace v1.4.0 Ruling 2 implementer note: "reading the scrollback_len delta is the canonical vt100 approach."

Both references replaced with accurate description of the read-back probe pattern: `set_scrollback(usize::MAX)`
then read `screen().scrollback()` to obtain the effective maximum available scrollback rows (vt100 clamps the
value internally and the read-back returns the actual maximum). This pattern is already documented in
`.factory/specs/research/tui-term-0.3.4-scrollback-api.md` and is what the implementation correctly uses.

No behavioral content changed. The clamp semantics (upper-bound to maximum available scrollback rows) are
identical; only the description of HOW to determine that maximum is corrected.

- SE-16d monotonicity: v1.4.1 timestamp 2026-06-21 — editorial patch; timestamp aligns with adversarial pass date. PASS.
