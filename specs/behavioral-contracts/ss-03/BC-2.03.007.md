...
3. On receiving `BinaryNotFound`, the error propagates as `EngineError::BinaryNotFound` →
   ...
   Canonical code: `"binary_not_found"` (SS-ipc.md §ServerToClient::Error taxonomy;
   SS-session-manager.md §session_error_to_code spawn-path arms).
...
7. On receiving `InvalidPath`, the error propagates as `EngineError::InvalidPath` →
   ...
   Canonical code: `"invalid_spawn_arg"` (SS-ipc.md §ServerToClient::Error taxonomy;
   SS-session-manager.md §session_error_to_code spawn-path arms).
...
| Architecture Source | SS-engine-module-v2-delta.md v1.6.0 §EngineError (new in v1A) + §Semantic contract (IMP-5 InvalidPath correction) + §spawn_recipe() two-pronged null-byte detection (C34-001) + §Phase Compatibility (I27-001 Model A: spawn_recipe() called daemon-side inside spawn_session()); SS-ipc.md §ServerToClient::Error taxonomy (codes `"binary_not_found"` and `"invalid_spawn_arg"` — I12-001); SS-session-manager.md v2.18.0 §session_error_to_code spawn-path arms (EngineError bridge — I12-001; Model A reachability confirmed — I27-001) |
...
- SE-16d monotonicity: 1.2.10 > 1.2.9. PASS.
...
- SE-16d monotonicity: v1.2.9 > v1.2.8. PASS.
...
- SE-16d monotonicity: v1.2.6 timestamp 2026-06-19 >= v1.2.5 timestamp auto. PASS.
