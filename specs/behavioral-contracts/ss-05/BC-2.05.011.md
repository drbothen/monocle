...
3. On receipt of `ScrollbackDumpComplete`, the TUI:
   ...
   c. Reconstructs the vt100 screen from the accumulated `SerializedCell` rows (per
      SS-session-manager.md v2.3.0 §Screen-state transfer on Attach reconstruction paths).
...
4. `PtyReset` is a rare event — it fires only on an actual PTY byte drop (channel `SendError`,
   ...
   fires. See SS-session-manager.md v2.3.0 §PTY reader thread §Forced parser-reset protocol.
...
5. The three variants are dispatched via the same per-client isolated send buffer (capacity 64
   per SS-ipc.md v1.21.0 §TUI IPC Read Loop Pattern) as `PtyOutput` (BC-2.05.009 Invariant 3b).
...
6. **`dump_in_progress` → buffer live PtyOutput in `pending_pty_bytes`, replay on Complete.**
   ...
   SS-session-manager.md v2.3.0 §Screen-state transfer on Attach step 5e (I3-003 fix).
...
| Architecture Source | SS-daemon-wiring-v2-delta.md v1.10.0 §5b (ScrollbackChunk/ScrollbackDumpComplete fan-out; I3-003 resume-after-snapshot); §5c (PtyReset fan-out); SS-session-manager.md v2.3.0 §Screen-state transfer on Attach (step 5d-5e: buffer PtyOutput during dump, replay on Complete); SS-ipc.md v1.21.0 §`ClientToServer::AttachSession` (I3-004 — TUI sends AttachSession not DaemonToHost::Attach); ADR-0010 v1.6.0 §pty-bytes-over-shared-uds-ipc (shared UDS decision + chunked protocol) |
