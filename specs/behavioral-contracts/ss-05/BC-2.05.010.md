...
3. On success: `ServerToClient::SessionStateChanged { session_id, new_state }` broadcast BEFORE
   ...
   PC-3 / SS-daemon-wiring-v2-delta.md v1.11.3 §3b). The `SessionStateChanged` event reflects
...
5. **No-silent-failure EXCEPTION (ResizePane carve-out):** `ResizePane` is the ONLY
   ...
   SS-session-manager.md v2.6.0 §ResizePane special rule (lines 572-576).
...
3. On success: `ServerToClient::SessionStateChanged { session_id, new_state: Detached }` broadcast
   ...
   BC-2.08.008 PC-3 / SS-daemon-wiring-v2-delta.md v1.11.3 §3b).
4. On failure: `ServerToClient::Error { code: <detach-path-code>, message: ... }` sent to the
   ...
   `session_error_to_code(IpcOp::Detach, &e)` (SS-session-manager.md v2.6.0 §Mapping table line 440):
...
5. Use cases: (a) TUI re-attach after `PtyReset` (BC-2.05.011 PC-3c), (b) user explicitly
   ...
   Per SS-ipc.md v1.23.2 §`ClientToServer::AttachSession`.
...
6. **No-silent-failure invariant:** The daemon IPC handler MUST NOT return `Ok(())` at the
   ...
   **Exception — ResizePane only:** `ResizePane` is the sole carve-out. After the zero-dimension
   ...
   Per SS-ipc.md v1.23.2 lines 1515/389 (normative citation) and SS-session-manager.md v2.6.0 lines
...
| Architecture Source | SS-daemon-wiring-v2-delta.md v1.11.3 §IPC handler — new ClientToServer variants (including AttachSession; `ClientToServer::SpawnSession { opts: SpawnOptions }` wire variant under Model A — I27-001); SS-ipc.md v1.23.2 §`ClientToServer::SpawnSession { opts: SpawnOptions }` (Model A wire variant — I27-001); SS-ipc.md v1.23.2 §`ClientToServer::AttachSession` (I3-004 — TUI re-attach; replaces incorrect "TUI sends DaemonToHost::Attach" description); SS-ipc.md v1.23.2 §`ServerToClient::Error` — Error variant + code taxonomy (`spawn_failed`, `binary_not_found`, `invalid_spawn_arg`, `spawn_unsupported`, `sidecar_write_failed`, `session_id_collision`, `session_not_found`, `attach_failed`, `kill_failed`, `rename_failed`, `invalid_request`, `session_not_ready`) — 12 codes as of v1.23.2; `spawn_unsupported` added F-P44-IMP-001; `session_not_ready` added F-P50-001; wire producer is DetachSession arm only (F-P51-001); added by architect in Pass-6 parallel track (C6-001); SS-session-manager.md v2.6.0 §session_error_to_code (spawn-path arms — Model A reachability for binary_not_found/invalid_spawn_arg confirmed I27-001; UnsupportedOperation → "spawn_unsupported" arm added F-P44-IMP-001; SessionNotReady producer-set: DetachSession arm only, resize excluded F-P51-001); SS-engine-module-v2-delta.md v1.6.0 §SessionError (9-variant enum including SessionNotReady — F-P50-001) |
