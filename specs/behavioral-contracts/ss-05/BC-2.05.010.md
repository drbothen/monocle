...
5. **No-silent-failure EXCEPTION (ResizePane carve-out):** `ResizePane` is the ONLY
   ...
   SS-session-manager.md v2.18.0 §ResizePane special rule.
...
- Architecture Source pin updated: SS-session-manager.md v2.17.0 → v2.17.1. Body citation at §ResizePane IPC arm carve-out updated to v2.17.1. No behavioral content changed.
- SE-16d monotonicity: v1.9.11 > v1.9.10. PASS.
...
**SS-session-manager v2.15.1 → v2.16.0 Architecture Source pin cascade (architect ruling errata)** (2026-06-21T00:00:00Z):
- Architecture Source pin updated: SS-session-manager.md v2.15.1 → v2.16.0.
- v2.16.0 fixes a stale Ruling A row that had incorrectly assigned the daemon-side ResizePane
  IPC handler and `resize_session()` to S-042 (should be S-047). No behavioral content changed.
  S-042 ResizePane carve-out WARN-drop rule is unaffected.
- SE-16d monotonicity: v1.9.8 > v1.9.7. PASS.
...
- SE-16d monotonicity: v1.9.5 timestamp 2026-06-19 >= v1.9.4 timestamp auto. PASS.
