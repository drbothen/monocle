...
3b. **Per-client send buffer isolation:** Each connected TUI client has a dedicated
   `mpsc::channel::<ServerToClient>(64)` (capacity 64, per SS-ipc.md v1.18.0 §TUI IPC Read
...
| Architecture Source | SS-daemon-wiring-v2-delta.md v1.7.0 §broker fan-out — PtyOutput messages; ADR-0010 v1.5.0 §pty-bytes-over-shared-uds-ipc; SS-session-manager.md v1.9.0 §PTY reader thread; SS-ipc.md v1.18.0 §TUI IPC Read Loop Pattern (per-client channel capacity 64, rationale) |
