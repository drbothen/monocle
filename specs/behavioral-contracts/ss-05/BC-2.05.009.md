...
3b. **Per-client send buffer isolation:** Each connected TUI client has a dedicated
   `mpsc::channel::<ServerToClient>(64)` (capacity 64, per SS-ipc.md v1.23.1 §TUI IPC Read
   ...
   isolation model per SS-daemon-wiring-v2-delta.md v1.11.2 §5d.
...
| Architecture Source | SS-daemon-wiring-v2-delta.md v1.11.2 §broker fan-out — PtyOutput messages; ADR-0010 v1.6.0 §pty-bytes-over-shared-uds-ipc; SS-session-manager.md v2.5.1 §PTY reader thread; SS-ipc.md v1.23.1 §TUI IPC Read Loop Pattern (per-client channel capacity 64, rationale) |
