...
6. `DaemonState.ring` (`Arc<RingBuffer>`) is initialized (invariant: step 4 of the daemon
   start sequence per BC-2.04.001 PC-4).
...
The handler calls `EngineModule::on_hook(HookType::PreToolUse, session_id, &payload)` on
each registered `EngineModule` in the registry (Phase 1: exactly one module, `ClaudeCodeModule`).
The returned `HookDecision` is:
- `HookDecision::Allow` — proceed to PC-6 immediately.
- `HookDecision::Block { reason }` — proceed to PC-6 with a block response.
...
decision arrives via the oneshot channel), the handler produces a fail-closed `HookResponse`
(deny with `reason: "timeout"`) and returns HTTP 200. The decision is NOT held beyond the
300ms budget under any circumstances.
...
- `HookDecision::Block { reason }` → `{"decision": "block", "reason": "<reason>"}`.
- Timeout → `{"decision": "block", "reason": "timeout"}` (fail-closed for writes).
...
| EC-072 | `HookDecision::Defer` returned but no TUI clients are connected | `PermissionPromptQueued` IPC push is a no-op (zero connected clients); oneshot receiver awaits timeout; handler returns fail-closed block after 300ms |
...
| EC-075 | `DaemonState.ring` append blocks longer than the remaining timeout budget | The `tokio::time::timeout` wrapping the handler body fires; ring append is cancelled; handler returns fail-closed block; WARN logged |
...
| Valid PreToolUse JSON body, auth OK, module returns `Block { reason: "policy" }` | HTTP 200, `{"decision": "block", "reason": "policy"}`, event bus receives 1 event | happy-path |
...
| Valid PreToolUse JSON body, auth OK, module returns `Defer`, 300ms elapses | HTTP 200, `{"decision": "block", "reason": "timeout"}`, event published (pre-timeout path) | edge-case |
...
| VP-TBD | 300ms timeout fires and returns fail-closed block when module hangs | integration |
