...
- **Forward-compatibility contracts (locked pre-Phase-1 per human authorization):**
  ...
  - **JSONL ring format versioning:** Every JSONL event record carries `format_version: u32 = 1` as first key. Phase 2 trigger-trace can read Phase 1 ring history; version field allows future format evolution. See `SS-daemon-lifecycle.md` v1.0.5 §JSONL Ring Buffer.
  - **Versioned auth token prefix:** Auth token format `monocle-v1:<64-char-hex>`. Phase 4 federation can introduce OAuth2 (`Bearer ...`) tokens without colliding with Phase 1 local tokens. Validation rule: reject non-prefix tokens with HTTP 401. See `SS-daemon-lifecycle.md` v1.0.5 §Daemon Lifecycle Protocol.
