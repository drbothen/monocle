...
| 1.4.24 | 2026-05-17T20:00:00Z | product-owner (T-128n Part 2 — F-R105 closure chain Round 4: ADR-0005 dual-accept propagation to brief lines 116 + 239) | **T-128n Part 2 — ADR-0005 dual-accept auth header propagation.** Two body sites updated to reflect architect's ADR-0005 decision (dual-accept at router middleware: canonical `X-Monocle-Authorization` + compatibility alias `X-Claude-Code-Ide-Authorization`). **SE-17f line 116 (Phase 1 constraints hook-ingestion bullet):** Before: `auth via X-Claude-Code-Ide-Authorization header`. After: `auth via dual-accept header per ADR-0005: canonical X-Monocle-Authorization: monocle-v1:<64-hex> (monocle-aware tools) takes priority; X-Claude-Code-Ide-Authorization: <64-hex> (real Claude Code compatibility alias, raw token no prefix) accepted as fallback with WARN-level deprecation log`. **SE-17f line 239 (Phase 1 Success Criteria — Hook protocol parity row Target cell):** Before: `...dual-accept auth per ADR-0005 — canonical path X-Monocle-Authorization tested AND compatibility alias path X-Claude-Code-Ide-Authorization tested; both paths validated by integration tests in auth_header_rejection.rs`. After: as per updated cell (both canonical and alias test paths now explicitly called out). No behavioral scope change — ADR-0005 was already accepted by architect; this is a brief propagation of the accepted decision. **D-042 brief sweep result (R105 Round 4).** grep -nE `SS-[a-z-]*\.md v[0-9]` run across full `.factory/specs/product-brief.md`. All SS-* current-pointer hits: `SS-daemon-lifecycle.md v1.0.7` inline citations at body lines 177-178 are historical pinpoints (introduced in R53 at that version; current frontmatter is v1.0.29 per CLAUDE.md §Architectural Authority entry 1 — the inline citations are leave-alone per sweep protocol). `SS-engine-module.md v1.1.15` at line 253 confirmed CURRENT per v1.4.22 trace. No additional stale current-pointers found. SE-16d monotonicity PASS: 2026-05-17T20:00:00Z > prior 2026-05-14T07:50:10Z (v1.4.23). |
