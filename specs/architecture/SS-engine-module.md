...
**§Trace v1.1.27** (2026-06-13T00:00:00Z) — C30-001/C30-002/I30-001 closure: `SpawnOptions`, `SpawnRecipe`, `SessionSnapshot`, `SerializedCell`, `PermissionPromptPayload` added to audit table:

- NORMATIVE (C30-001 CRITICAL — SpawnOptions): Added `SpawnOptions` row. Defined in `monocle-core`; constructed cross-crate by `monocle-tui` (SessionCreation wizard) and `monocle-runtime` (daemon IPC handler). TUI-side: `SpawnOptions::for_spawn_request(project_root, worktree_root, harness_id, profile_id, ccr_base_url) -> Self`. Daemon-side: `with_daemon_fields(self, session_id, hooks_settings_path) -> Self` (consuming builder; replaces E0639-violating `..opts` functional-update in SS-daemon-wiring-v2-delta.md §3). Both constructors defined in SS-engine-module-v2-delta.md §SpawnOptions and mirrored byte-for-byte in SS-session-manager.md §SpawnOptions.
- NORMATIVE (C30-001 CRITICAL — SpawnRecipe): Added `SpawnRecipe` row. Defined in `monocle-core`; constructed cross-crate by `monocle-runtime` (`ClaudeCodeModule::spawn_recipe()`). Constructor: `SpawnRecipe::new(binary, args, env, cwd) -> Self`. The `ClaudeCodeModule::spawn_recipe()` implementation spec in SS-engine-module-v2-delta.md updated to use `SpawnRecipe::new(...)` instead of a struct literal. `#[non_exhaustive]` was previously called "harmless" in SS-engine-module-v2-delta.md §Phase Compatibility — that text was wrong; the cross-crate construction makes E0639 applicable; text corrected.
- NORMATIVE (C30-002 CRITICAL — SessionSnapshot): Added `SessionSnapshot` row. Defined in `monocle-ipc`; constructed cross-crate by `monocle-runtime` (daemon) when building `InitialState.sessions` and `SessionListUpdate.sessions`. Constructor: `SessionSnapshot::new(10 positional fields) -> Self` + `.with_degraded(reason) -> Self` builder for the I3-009 degraded-env path. Defined in SS-ipc.md §Supporting Types (v1.19.0 → v1.20.0).
- NORMATIVE (C30-002 CRITICAL — SerializedCell): Added `SerializedCell` row. Defined in `monocle-ipc`; constructed cross-crate by the `monocle-session-host` binary when serializing `vt100::Cell` values for `HostToDaemon::ScrollbackChunk`. Constructor: `SerializedCell::new(ch, fg, bg, attrs) -> Self`. Defined in SS-ipc.md §Supporting Types (v1.20.0).
- NORMATIVE (I30-001 IMPORTANT — PermissionPromptPayload adjudicated as cross-crate construction): Added `PermissionPromptPayload` row. Defined in `monocle-ipc`; the daemon (`monocle-runtime`, `src/hooks/pre_tool_use.rs`) actively constructs this struct when building `ServerToClient::PermissionPromptQueued { payload }`. The TUI only serde-deserializes received payloads — no TUI-side construction site. Adjudication: NOT serde-deserialize-only exempt (unlike `SessionStartEvent` whose sole production construction occurs inside `monocle-core` itself). Constructor: `PermissionPromptPayload::new(prompt_id, session_id, tool_name, tool_input, old_content, new_content) -> Self`. Defined in SS-ipc.md §Supporting Types (v1.20.0).
- NORMATIVE (exhaustive sweep — v1A spec docs): Sweep of SS-engine-module-v2-delta.md, SS-session-manager.md, SS-ipc.md, SS-daemon-wiring-v2-delta.md for all `#[non_exhaustive]$` occurrences followed by `pub struct`. Results: `SpawnRecipe` (SS-engine-module-v2-delta.md:64), `SpawnOptions` (SS-engine-module-v2-delta.md:92, SS-session-manager.md:1097 — same struct, two spec mirrors), `SessionSnapshot` (SS-ipc.md:630), `SerializedCell` (SS-ipc.md:689), `PermissionPromptPayload` (SS-ipc.md:732). All five are now covered. No additional `#[non_exhaustive] pub struct` instances found in v1A spec docs. Enums in SS-ipc.md (lines 170, 433) and SS-session-manager.md (lines 159, 739, 755) remain exempt per audit table introductory note.
- SE-16d PASS: 2026-06-13T00:00:00Z > chain high-water 2026-05-28T10:00:00Z (monotonic).
