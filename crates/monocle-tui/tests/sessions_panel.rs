...
// ---------------------------------------------------------------------------
// F-S025-ADV5-BLOCKER-001 — BC-2.06.005 canonical column order
// ---------------------------------------------------------------------------
//
// The BC-2.06.005 canonical test vector is:
...
/// F-S025-ADV5-BLOCKER-001 / BC-2.06.005: canonical row — jitter-tolerant integration.
///
...
fn test_bc_2_06_005_canonical_row_jitter_tolerant_integration() {
    ...
    assert!(
        ...
        "F-S025-ADV5-BLOCKER-001 / BC-2.06.005: rendered row must contain \
...
/// F-S025-ADV6-MED-001 / BC-2.06.005: verbatim match against canonical BC vector
...
///
/// Canonical BC vector (BC-2.06.005 test vector table):
///   `sess-001 ● monocle Active 437k — 03:47:00`
///
...
fn test_bc_2_06_005_canonical_row_verbatim_with_mocked_time() {
    ...
    // Verbatim assertion: EXACT canonical BC-2.06.005 test vector.
    // No substring tolerance. No jitter tolerance. Byte-for-byte equality.
    assert_eq!(
        row, "sess-001 \u{25CF} monocle Active 437k \u{2014} 03:47:00",
        "F-S025-ADV6-MED-001 / BC-2.06.005: format_session_row must produce \
    ...
    assert_eq!(
        uptime, "03:47:00",
        "BC-2.06.005: format_uptime_at(3h47m, now) must return exactly '03:47:00'"
    );
}

/// F-S025-ADV5-BLOCKER-001 / BC-2.06.005: session_id appears BEFORE icon.
///
...
fn test_bc_2_06_005_column_order_session_id_first() {
    ...
    assert!(
        id_pos < icon_pos,
        "F-S025-ADV5-BLOCKER-001 / BC-2.06.005: session_id must appear BEFORE icon; \
         session_id at {id_pos}, icon at {icon_pos} in:\n{rendered}"
    );
}

/// F-S025-ADV6-LOW-001 / BC-2.06.005: every inter-column boundary uses
