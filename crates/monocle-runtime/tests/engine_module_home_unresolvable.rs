...
/// AC-005 / BC-2.03.003 PC-1: When HOME, USERPROFILE, HOMEDRIVE, and HOMEPATH are all
/// unset, `metadata()` must return `Err(EngineMetadataError::HomeUnresolvable)` with no
...
/// AC-005 / BC-2.03.003 PC-1: When HOME, USERPROFILE, HOMEDRIVE, and HOMEPATH are all
/// unset, `enrich()` must return `Err(EngineMetadataError::HomeUnresolvable)` with no
/// default path substitution.
///
/// Uses `temp_env::async_with_vars` for hermetic env isolation — restores original values
/// after the closure, even on panic.
