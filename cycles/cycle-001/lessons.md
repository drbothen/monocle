...
### L-W6-S025-020: Correcting a stale spec claim requires RE-DERIVING the replacement from the implementation, not asserting it — an unverified correction over-claiming behavior never wired creates a partial-fix regression requiring a second adjudication cycle [codified]

**Date:** 2026-05-31
**Severity:** codified (convergence process pattern — observed at D-218, Pass 39 HOLD)
**Origin:** S-025 adversarial Pass 39 verdict: HOLD 0/3 (counter STAYS). Pass 38 correctly identified that AC-001/AC-009 carried a stale "Esc quits Dashboard" claim and fixed it. However, the Pass-38 replacement prose introduced a new false claim: "Esc returns from Fullscreen/Overlay to Dashboard." The S-025 skeleton wires ONLY the Enter→Fullscreen transition arm; the fullscreen VIEW and its Esc-exit binding (BC-2.06.007 PC-5 via Action::ExitFullscreen) are deferred to the Sessions Panel fullscreen-view story. In S-025 scope, Esc is identity/no-op in Fullscreen and Overlay modes — not a return-to-Dashboard action. The replacement was asserted from first principles about what "Esc should do" rather than re-derived from the actual wiring in dispatch_key_event.

**Pattern:** When an adversarial pass identifies a stale spec claim and a specialist (story-writer or implementer) produces corrective prose, the corrective prose is itself a new spec claim. If this replacement is authored by inference ("Esc should return to Dashboard from Fullscreen — that's the obvious behavior") rather than by tracing the actual code path ("dispatch_key_event in Fullscreen mode: Esc → identity/no-op; Action::ExitFullscreen is not wired in S-025"), the replacement may over-claim behavior that is not yet implemented. The adversary in the next pass reads the replacement as a behavioral commitment, verifies it against the implementation, and finds the same class of defect: spec claims something the code does not do. The net result is a second-order content defect created by the corrective act.

This is distinct from the original L-W6-S025-019 lesson. L-W6-S025-019 concerns perimeter-anchored passes missing content defects by not re-deriving from source. L-W6-S025-020 concerns the CORRECTION PROCESS itself: even when the adversary correctly identifies the defect class (stale Esc claim), the specialist's replacement prose must be grounded in the actual implementation, not in what the implementation "should" do or "will eventually" do.

**Rule:** When correcting a stale spec claim, the REPLACEMENT must be re-derived from the implementation by tracing the actual code path, not asserted from design intent or from what the behavior should eventually be. The correct process is:
1. Read the implementation (the binding table, dispatch_key_event, state machine) for the mode in question.
2. State ONLY what the code actually does in that mode today.
3. If the design calls for behavior that is not yet wired (e.g., fullscreen-Esc-exit deferred to a later story), the spec must say "deferred to [story name]" — not assert the deferred behavior as current.
4. The replacement undergoes the same adversarial re-derivation as the original claim.

An asserted replacement that describes future/intended behavior as current behavior is a partial-fix regression: it closes one defect while opening another of the same class. The adversary will find it in the next pass. The fix is to anchor every spec claim about current behavior in an explicit code-path trace, and to explicitly defer future behavior with a story anchor rather than asserting it.
