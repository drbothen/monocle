...
durable_task_register:
  outstanding:
    ...
    - id: "F-S025-ADV26-OBS-001"
      subject: "[scope-extension TRIPWIRE] META-pattern structural-spec drift layer — CLOSED (ADR-0008 ratified)"
      status: closed
      detail: "CLOSED (D-206): TRIPWIRE FIRED at Pass 27 (2nd structural-spec drift instance — story-body type-name Vec<SessionState> vs Vec<EnrichedSession>). Architect cb68158 ratified ADR-0008 (Option B — Structural-Claim Discipline; POL-12). Species architecturally bound. 2 structural-spec drift instances confirmed: Pass 26 (module-doc column table) + Pass 27 (story-body type-name). ADR-0007 focused on literal-pin discipline; ADR-0008 owns structural-claim discipline. Task #9 m.6 CLOSED."
      blocking: false
    - id: "F-S025-ADV27-MED-001"
      subject: "Story body type-name drift Vec<SessionState> vs canonical Vec<EnrichedSession> (META 9th + structural-spec #2)"
      status: closed
      detail: "CLOSED (D-206) via story-writer 30fb391 (S-025 v1.9→v1.10; lines 144+228 Vec<SessionState> → Vec<EnrichedSession>; STORY-INDEX v5.14→v5.15; sweep-wider clean — line 164 disclaimer preserved) + architect cb68158 strategic ratification (ADR-0008 v1.0.0). Cross-story S-028:63,147 deferred per BC-5.39.002 PC2 (Task #9 m.8)."
      blocking: false
    - id: "ADV22-PROC-001"
    ...
    - id: "ADV25-PROC-001"
      ...
      blocking: false
    - id: "Task-9-m7-POL12-devops"
      subject: "Task #9 m.7 — devops POL-12 implementation per ADR-0008 §Implementation Plan (HIGH priority post-S-025-merge)"
      status: pending
      detail: "ADR-0008 §Implementation Plan (D-206; cb68158). devops-engineer dispatch needed: CI enforcement for structural-claim discipline (type names, table shapes, postcondition counts, enum variants). Mechanism: table-parser separate from POL-11 regex. Priority HIGH. Target: Task #9 batch post-S-025-merge. Anchored to ADR-0008 v1.0.0."
      blocking: false
    - id: "Task-9-m8-S028-cross-story"
      ...
      status: pending
      detail: "F-S025-ADV27-MED-001 sweep-wider: S-028 lines 63+147 still reference Vec<SessionState> type drift. Deferred per BC-5.39.002 PC2 (cross-story propagation). story-writer sweep required at wave-gate. Anchored to Task #9 wave-gate batch."
      blocking: false
    - id: "F-S025-ADV16-CODIFY-001"
  ...
  process_discoveries:
    ...
    - "9TH META-PATTERN INSTANCE + TRIPWIRE CLOSURE (D-206 — Pass 27): structural-spec drift species confirmed at 2 instances (Pass 26: module-doc column table; Pass 27: story-body type-name Vec<SessionState> vs Vec<EnrichedSession>). Tripwire D-205 m.6 OBS-001 FIRED at 2-instance threshold per orchestrator calibration. Architect ratified ADR-0008 (Option B over Option A — incompatible algorithms: POL-11 regex for literal-pin vs POL-12 table-parser for structural-claim cannot be bundled into one hook without unmaintainable complexity). 2 ADRs for 2 distinct detection mechanisms is correct factoring. Escalation ladder now spans 9 layers: test-assertion → struct-metadata → impl-worktree → spec-filename → BC-body → sibling-artifact-directory → code-citation BC-version pin → module-doc structural-spec TABLE → story-body type-name."
    - "THREE-TRACK STRATEGY TEMPLATE (D-206 — EXECUTED 3RD TIME): Passes 25 + 26 + 27 all executed the Three-Track Strategy template (architect strategic + story-writer/implementer tactical + state-manager logging) in a single coordinated round. Cycle-001 has now validated this template 3 times. The pattern for tripwire-fired events is: parallel dispatch closes strategic+tactical+logging in a single coordinated round without multi-session cycling. Template is cycle-documented and cross-cycle reusable. NOTE: Pass 26 used 2-track (architect + implementer; no story-spec changes); Pass 25 + 27 used 3-track (architect + story-writer/implementer + state-manager). When story-spec changes are needed in a tripwire event, dispatch story-writer explicitly."
    - "META-PATTERN EMPIRICAL MODEL (D-206 — RESOLVED): defect species at scale manifest in sub-species with distinct mechanisms. At 9 instances: 2 sub-species confirmed (literal-pin + structural-claim). Strategic interventions bound their target sub-species (ADR-0007 for literal-pin; ADR-0008 for structural-claim). Orchestrator's tripwire 3-instance discipline catches sub-species emergence before unbounded recurrence — calibration validated at Pass 27 (2nd structural-spec drift instance triggered tripwire; no 3rd instance needed because mechanism was clear after 2). The Three-Perimeter Scope Contract + S-7.02 codification rule is the correct governance framework for this class of failure."
next_session_resume_protocol: |
  S-025 PASS 27 MED (D-206) — META 9TH INSTANCE (structural-spec drift #2: story-body type-name) — 3-TRACK CLOSURE — TRIPWIRE D-205 m.6 CLOSED — COUNTER 0/3 — PASS 28 PENDING — STATE v6.55

  STATE: develop @ 7a52041. 26/33 done (156/195 pts). 852+ tests. S-025 worktree HEAD: 2d1188f (unchanged) on feature/S-025-tui-skeleton-sessions. Factory-artifacts: post-cb68158+30fb391 after this burst.
  COUNTER: 0/3 (holds — Pass 27 found 1 MED; closed via 3-track strategy; counter does NOT advance until Pass 28 dispatches at post-fix HEAD).
  TRAJECTORY: 5→4→3→2→4→H→M→0→M→M→H→C→L→N(14)→C(15)→M(16)→N(17)→M(18)→M(19)→M(20)→C(21)→M(22)→REMEDIATED→M(23)→REMEDIATED→M(24)→M(25)+ADR-0007→M(26)→M(27)+ADR-0008.
  TOTAL RESETS: 9 (Passes 9, 16, 18, 22, 23, 24, 25, 26, 27). CONVERGENCE-ATTEMPT #10 — Pass 28 pending.
  MSRV: Phase 1 = 1.88 (time 0.3.47 floor). Phase 3 = 1.92. CLAUDE.md line 18 needs human update (F-S025-PATH-B-CLAUDE-MD).

  PASS 27 OUTCOME (D-206):
    F-S025-ADV27-MED-001: S-025:144 + S-025:228 `Vec<SessionState>` drift — canonical is `Vec<EnrichedSession>` (SS-tui.md:845 + app.rs:130 + BC-2.06.005). SessionState exists as monocle-runtime::hooks runtime-internal enum — wrong type entirely. META-pattern 9th instance + structural-spec drift instance #2.
    TRIPWIRE D-205 m.6 OBS-001: FIRED (2nd structural-spec drift instance reached threshold). CLOSED.
    3-TRACK CLOSURE (proven template — 3rd execution: Pass 25 + 26 + 27):
      - Architect: cb68158 — ADR-0008 v1.0.0 (Option B — Structural-Claim Discipline; POL-12; canonical source registry; CI enforcement spec). ADR-0007 v1.0.1→v1.0.2 (forward-reference paragraph only). SS-conventions v1.32.1→v1.32.2 (new §Structural-Claim Discipline section). ARCH-INDEX v1.0.17→v1.0.18 (ADR-0008 registered).
      - Story-writer: 30fb391 — S-025 v1.9→v1.10; lines 144+228 Vec<SessionState> → Vec<EnrichedSession>; STORY-INDEX v5.14→v5.15; sweep-wider clean (line 164 disclaimer preserved). Cross-story S-028:63,147 deferred to wave-gate (Task #9 m.8).
      - State-manager: D-206 — this burst.
    STRATEGIC OUTCOME: 9-instance META-pattern now bound by 2 ADRs. ADR-0007 owns literal-pin discipline (POL-11). ADR-0008 owns structural-claim discipline (POL-12). Both species architecturally resolved.

  IMMEDIATE NEXT ACTIONS:
    1. Pass 28 adversary at post-fix HEAD (2d1188f worktree unchanged; 30fb391 + cb68158 + this D-206 burst on factory-artifacts). Target: 0/3 → 1/3 (10th convergence attempt — FIRST with both META-pattern sub-species architecturally bound).
    2. ADR-0007 + ADR-0008 strategic coverage now complete. Both literal-pin + structural-claim species covered. Pass 28 should find no further META-pattern instances.
    3. If Pass 28 NITPICK_ONLY-CLEAN: counter 0/3→1/3. Convergence forecast: 3/3 at Pass 30 if Passes 28+29+30 all clean.
    4. After S-025 convergence 3/3: rebase → develop; resolve TODO(S-023-merge); demo-recorder; PR #28; S-026 dispatch.
    5. Task #9 m.1-m.5 (ADR-0007 impl) + m.7 (ADR-0008 devops POL-12) + m.8 (S-028 cross-story): dispatch post-convergence.

  CRITICAL FILES FOR PASS 28 ADVERSARY (read in order):
    1. .factory/STATE.md (v6.55); 2. adversarial-pass-27.md; 3. adversarial-pass-26.md;
    4. adversarial-pass-25.md; 5. adversarial-pass-24.md; 6. adversarial-pass-23.md;
    7. adversarial-pass-22.md; 8. adversarial-pass-21.md; 9. adversarial-pass-20.md;
    10. adversarial-pass-18.md; 11. adversarial-pass-16.md; 12. adversarial-pass-15.md;
    13. adversarial-pass-12.md; 14. architect-decisions-pass-1.md; 15. architect-decisions-pass-2.md;
    16. text-style-adjudication.md; 17. red-gate-log.md;
    18. .factory/stories/S-025-tui-skeleton-sessions.md (v1.10 post-fix);
    19. .factory/specs/architecture/adr/ADR-0007.md (v1.0.2 — forward-ref to ADR-0008);
    20. .factory/specs/architecture/adr/ADR-0008.md (v1.0.0 — Structural-Claim Discipline);
    21. CLAUDE.md (project principles — production-grade default).
  ...
  RECURRENCE WATCH:
    ...
    Structural-spec drift species (NEW — Pass 26 MED-001; BOUNDED — Pass 27 ADR-0008): 2 instances (Pass 26: module-doc column table; Pass 27: story-body type-name). TRIPWIRE D-205 m.6 OBS-001 CLOSED. ADR-0008 (POL-12) adopted. Species architecturally bound.
    1/3→2/3 transition failure count: 9 (Passes 9, 16, 18, 22, 23, 24, 25, 26, 27 — all MED, all different classes). STRATEGIC RESOLUTION ADOPTED: ADR-0007 (literal-pin species; D-204) + ADR-0008 (structural-claim species; D-206). Pass 28 is FIRST attempt with BOTH META-pattern sub-species architecturally bound.
    META-PATTERN: 9 instances confirmed. BOTH sub-species strategically resolved.
      9 instances confirmed. Species root: authoring-time documentation drifts as canonical evolves.
      Escalation ladder: test-assertion → struct-metadata → impl-worktree → spec-filename → BC-body → sibling-artifact-directory → code-citation BC-version pin → module-doc structural-spec TABLE → story-body type-name.
      ADR-0007 strategic intervention: VALIDATED for literal-pin target species (instances 18, 23, 24, 25). COMPLETE.
      ADR-0008 strategic intervention: ADOPTED for structural-claim species (instances 26, 27). COMPLETE.
      CODIFY-001 SUNSET: ADR-0007 supersedes per-pass enumeration with structural fix.
      ADR-0007 v1.0.2 (cb68158) — forward-reference to ADR-0008 added.
      ADR-0008 v1.0.0 (cb68158) — Structural-Claim Discipline; POL-12.
      Full sweep protocol (D-198.2 + D-201 + D-202 + D-203): PRESERVED for Task #9 m.1 CI implementation reference.

  ARTIFACT VERSIONS (D-206 / Pass 27 closure — post-3-track):
    ...
    SS-conventions-anti-patterns v1.32.2 (UPDATED — new §Structural-Claim Discipline section; cb68158). SS-tui v1.8.2. SS-config v1.3.0.
    ADR-0006 v1.2. ADR-0007 v1.0.2 (UPDATED — forward-reference to ADR-0008; cb68158). ADR-0008 v1.0.0 (NEW — Structural-Claim Discipline; POL-12; cb68158). ARCH-INDEX v1.0.18 (UPDATED — ADR-0008 registered; cb68158).
    S-025 v1.10 (UPDATED — Vec<SessionState> → Vec<EnrichedSession> lines 144+228; 30fb391). S-026 v1.8.
    BC-2.03.001 v1.0.6. S-001 v1.9. S-003 v1.8. S-014 v1.5. S-015 v1.7.
    S-024 v1.5. S-027 v1.4. S-028 v1.3. S-029 v1.2. S-031 v1.1.
    holdout-scenarios v1.5. STORY-INDEX v5.15 (UPDATED — S-025 v1.10; 30fb391).
    Worktree: 2d1188f unchanged (feature/S-025-tui-skeleton-sessions). Factory-artifacts: post-cb68158+30fb391+D-206-burst. CI confirmed 9/9 green on 2d1188f.
...
| 3 TDD Implementation | IN PROGRESS — Wave 6 2/4 done; S-025 Pass 27 MED (D-206); counter 0/3; 3-track closure (architect cb68158 ADR-0008 + story-writer 30fb391 + state-manager D-206); META 9th instance; TRIPWIRE D-205 m.6 CLOSED; Pass 28 pending | 2026-05-28 | Wave 1+2+3 DONE (83 pts, 447 tests, all 6 gates). Wave 4 GATE PASSED (D-175): 634 tests. Wave 5 GATE PASSED (D-182): 753 tests, 0 failures, clippy clean, fmt clean. Wave 6: 2/4 done (S-022 8pts + S-023 5pts). 26/33 stories done (156/195 pts). S-025 Pass 27 MED: META-pattern 9th instance (structural-spec drift #2 — story-body type-name); TRIPWIRE D-205 m.6 CLOSED; ADR-0008 v1.0.0 ratified (Structural-Claim Discipline POL-12); ADR-0007 v1.0.2 (forward-ref); 3-track closure; 2 ADRs now bind META-pattern. Counter HOLDS 0/3. Trajectory: 5→4→3→2→4→H→M→0→M→M→H→C→L→N(14)→C(15)→M(16)→N(17)→M(18)→M(19)→M(20)→C(21)→M(22)→REMEDIATED→M(23)→REMEDIATED→M(24)→M(25)+ADR-0007→M(26)→M(27)+ADR-0008. |
| 4-7 | not-started | — | |
...
develop @ 7a52041. 852+ tests, 0 failures. 26/33 stories done, 156/195 pts (80%). Wave 5 gate PASSED (D-182). Wave 6 in progress: S-022 DONE (D-184, PR #27 @ c7540539), S-023 DONE (D-186, PR #29 @ 7a52041). S-025 Pass 27 MED (D-206): META-pattern 9th instance (structural-spec drift #2 — story-body type-name Vec<SessionState> vs Vec<EnrichedSession>). TRIPWIRE D-205 m.6 CLOSED. 3-track closure: architect cb68158 (ADR-0008 v1.0.0) + story-writer 30fb391 (S-025 v1.10) + state-manager D-206. Counter 0/3. Pass 28 pending. S-026 blocked on S-025. Resets: Passes 9, 16, 18, 22, 23, 24, 25, 26, 27 = 9 total. ADR-0007 + ADR-0008 strategic resolutions both adopted. 9-instance META-pattern fully bound by 2 ADRs.
...
| D-206 | S-025 Pass 27 MED — F-S025-ADV27-MED-001: story-body type-name drift Vec<SessionState> vs canonical Vec<EnrichedSession> (S-025:144+228); META-pattern 9TH instance; structural-spec drift #2. TRIPWIRE D-205 m.6 OBS-001 FIRED (2-instance threshold). 3-TRACK CLOSURE: architect cb68158 (ADR-0008 v1.0.0 Option B — Structural-Claim Discipline; POL-12; ADR-0007 v1.0.1→v1.0.2 forward-ref; SS-conventions v1.32.1→v1.32.2; ARCH-INDEX v1.0.17→v1.0.18) + story-writer 30fb391 (S-025 v1.9→v1.10 type-name fix; STORY-INDEX v5.14→v5.15; sweep-wider clean; S-028:63,147 cross-story deferred Task #9 m.8) + state-manager D-206. TRIPWIRE CLOSED. Counter HOLDS 0/3 (9th 1/3→2/3 failure). 9-instance META-pattern now bound by 2 ADRs: ADR-0007 (literal-pin POL-11) + ADR-0008 (structural-claim POL-12). Three-Track Strategy template VALIDATED 3RD TIME. Convergence forecast: 3/3 at Pass 30 if 28+29+30 all clean. STATE v6.54→v6.55. | 2026-05-29 | state-manager |
...
28 pinned production deps. **manifest v1.2.0**. **PRD v1.27.3**. **BC-INDEX v1.32** (72 numbered BCs + 41 DTU BCs = 113 total; all ss-03/04/05/06/07/dtu BCs at current Architecture Source canonical per D-202.1). **ARCH-INDEX v1.0.18** (7 subsystems; ADR-0007 + ADR-0008 rows added; cb68158). **SS-tui v1.8.2**. **SS-engine-module v1.1.26**. **SS-conventions-anti-patterns v1.32.2** (UPDATED — new §Structural-Claim Discipline section; cb68158). **ADR-0006 v1.2**. **ADR-0007 v1.0.2** (UPDATED — forward-reference to ADR-0008; cb68158). **ADR-0008 v1.0.0** (NEW — Structural-Claim Discipline; POL-12; cb68158). **BC-2.03.001 v1.0.6**. **STORY-INDEX v5.15** (33 stories, 195 pts; UPDATED — S-025 v1.10; 30fb391). **sprint-state v1.30** (26/33 done, 156/195 pts, 80%). **S-001 v1.9**. **S-003 v1.8**. **S-014 v1.5**. **S-015 v1.7**. **S-024 v1.5**. **S-025 v1.10** (UPDATED — Vec<SessionState> → Vec<EnrichedSession>; 30fb391). **S-026 v1.8**. **S-027 v1.4**. **S-028 v1.3**. **S-029 v1.2**. **S-031 v1.1**. **holdout-scenarios v1.5**. MSRV: Rust 1.88 (Phase 1-2, time 0.3.47 floor per RUSTSEC-2026-0009; original ratatui floor was 1.86); Rust 1.92 (Phase 3, wasmtime 44). 43 codified disciplines (SE-1..SE-23 + SE-40 candidate + ADR-0007 semantic-anchor discipline + ADR scope-boundary observation D-205 + ADR-0008 structural-claim discipline + Three-Track Strategy template D-206). Workspace crates: monocle-core, monocle-runtime, monocle-proto, monocle-test-harness, monocle (binary), monocle-config, monocle-ipc, xtask.
...
## §Trace v6.55 (D-206 — Pass 27 MED; META-9th instance (structural-spec drift #2 story-body type-name); TRIPWIRE D-205 m.6 CLOSED; 3-track closure; architect cb68158 ADR-0008 v1.0.0 + story-writer 30fb391 S-025 v1.10 + state-manager D-206; 9-instance META-pattern bound by 2 ADRs)

**Pass 27 MED finding (2026-05-29, D-206):** F-S025-ADV27-MED-001 (S-025:144+228 Vec<SessionState> vs canonical Vec<EnrichedSession>; META-pattern 9th instance; structural-spec drift #2 at story-body type-name layer). TRIPWIRE D-205 m.6 OBS-001 FIRED (2nd structural-spec drift instance). All findings CLOSED via 3-track strategy.

**3-TRACK CLOSURE (D-206):** Architect cb68158 (ADR-0008 v1.0.0 Option B — Structural-Claim Discipline; POL-12 for structural claims; ADR-0007 v1.0.1→v1.0.2 forward-reference only; SS-conventions v1.32.1→v1.32.2 new §Structural-Claim Discipline; ARCH-INDEX v1.0.17→v1.0.18 ADR-0008 registered) + story-writer 30fb391 (S-025 v1.9→v1.10; lines 144+228 Vec<SessionState> → Vec<EnrichedSession>; STORY-INDEX v5.14→v5.15; sweep-wider clean; S-028:63,147 cross-story deferred Task #9 m.8) + state-manager D-206 (this burst). Three-Track Strategy template executed 3rd time — validated as reusable pattern for tripwire-fired strategic interventions.

**ADR-0008 rationale (D-206):** Option B over Option A — POL-11 regex (literal-pin detection) and POL-12 table-parser (structural-claim detection) are incompatible algorithms that cannot be bundled into one hook without unmaintainable complexity. 2 ADRs for 2 distinct mechanisms is correct factoring at project scale. ADR-0007 owns POL-11 (literal version-pin freshness). ADR-0008 owns POL-12 (structural-claim discipline: type names, table shapes, postcondition counts, enum variants).

**Artifact versions bumped (D-206):** STATE v6.54→v6.55. ADR-0007 v1.0.1→v1.0.2 (cb68158 — forward-ref only). ADR-0008 v1.0.0 (NEW; cb68158). SS-conventions v1.32.1→v1.32.2 (cb68158). ARCH-INDEX v1.0.17→v1.0.18 (cb68158). S-025 v1.9→v1.10 (30fb391). STORY-INDEX v5.14→v5.15 (30fb391). Counter HOLDS 0/3. Trajectory appended: →M(27)+ADR-0008.
